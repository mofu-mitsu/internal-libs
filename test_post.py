# -*- coding: utf-8 -*-
from atproto import Client
import random
import os
from dotenv import load_dotenv
from pathlib import Path
import unicodedata
import re
import requests
from bs4 import BeautifulSoup
from urllib.parse import urlparse
import tempfile
import io
from urllib.parse import urlparse
from PIL import Image

# 環境変数読み込み
env_path = Path('.') / '.env'
load_dotenv(dotenv_path=env_path)

HANDLE = os.getenv('HANDLE')
APP_PASSWORD = os.getenv('APP_PASSWORD')

POST_MESSAGES = [
    """みりん、寂しくてキラキラ作っちゃった…♡  
フィードで会いに来てくれたら、幸せだよ🥺  
➡︎ https://bsky.app/profile/did:plc:pvy7w75qks44sybd5wwpthb6/feed/aaae5hbblng6a  
#みりんてゃ #地雷感情 #ふわもこ通信""",

    """MBTIとか認知機能の診断って、
選択肢を押すだけだとちょっと物足りない時ない？🥹

行動しながら認知機能を測る、
体感型の診断ツールつくったよ🧠

https://mofu-mitsu.github.io/cognitive-dive/

#MBTI #個人開発""",

    """認知機能ダイブ・シミュレーターっていう、
ちょっと変わったMBTI系ツール作ったよ🧠

直感、法則発見、色の見分け、反応とか…
行動から認知機能を見てく感じのやつ♡

https://mofu-mitsu.github.io/cognitive-dive/

#MBTI #みりんてゃ""",
    """小説とか漫画の設定、
気づいたらあちこち散らばってない？🥹

プロット・登場人物・世界観を
まとめて整理できる執筆ツール作ったよ📚

https://plot-palette.vercel.app/

#創作 #個人開発""",

    """Plot Palette っていう
小説・漫画のプロット管理ツール作ったよ🎀

起承転結、キャラ、世界観、年表、
伏線までまとめて見やすくできるの♡

https://plot-palette.vercel.app/

#創作支援 #個人開発""",

    """“話はあるのに整理できない…”
って時のための創作ツール置いとくね🫶

プロットもキャラ設定も
きれいに可視化できるやつだよ📚

https://plot-palette.vercel.app/

#小説 #みりんてゃ""",

    """伏線って、
張るのは楽しいのに回収管理で詰みがちだよね…？🥹

Plot Palette なら
未回収 / 回収済で見える化できるよ📚

https://plot-palette.vercel.app/

#創作 #みりんてゃ""",

    """世界観もキャラも年表も、
1か所で見られるとちょっと安心しない？☁️

執筆用の創作アトリエみたいなやつ作ったよ♡

https://plot-palette.vercel.app/

#創作ツール #個人開発""",

    """セリフとか情景のひらめきって、
あとで探せなくなること多くない？🥺

メモも検索もまとめて使える
執筆ツールあるよ📚

https://plot-palette.vercel.app/

#小説執筆 #みりんてゃ""",

    """縦書きでそのまま書ける創作ツール、
ちょっとロマンない？🥹📖

Plot Palette、
執筆からPDF出力までいけるよ♡

https://plot-palette.vercel.app/

#小説 #個人開発""",

    """プロット、キャラ、世界観、伏線…
全部まとめて管理したい創作勢へ🎀

Plot Palette、
かなり“整理しながら書ける”寄りのやつです📚

https://plot-palette.vercel.app/

#創作支援 #みりんてゃ""",

    """“書きたい”はあるのに
設定が迷子になる子へ🥹

Plot Palette っていう
創作のパレットみたいなツール作ったよ♡

https://plot-palette.vercel.app/

#創作 #個人開発""",

    """小説、漫画、ゲームの設定づくりって
整理できるだけでだいぶ呼吸しやすいよね☁️

創作支援ツール、置いとくね📚

https://plot-palette.vercel.app/

#創作ツール #みりんてゃ""",

    """質問に答えなくていい診断って、
ちょっと惹かれない？🥹✨

ただ“光”を探すだけで
内なる原石を鑑定する宝石診断つくったよ💎

https://crystal-origin.vercel.app/

#診断 #個人開発""",

    """CRYSTAL ORIGIN 〜わたしを紐解く宝石診断〜💎

直感のままに光を探すだけで、
自分の中の“原石”を見つける診断だよ♡

https://crystal-origin.vercel.app/

#宝石診断 #心理テスト""",

    """自分がもし宝石だったら、
どんな輝きなのか気にならない？🥹💎

質問に答えない、
ちょっと不思議な診断あるよ♡

https://crystal-origin.vercel.app/

#診断メーカー #心理テスト""",

    """言葉で説明する前に、
光とか色で“自分っぽさ”感じる時あるよね☁️

そんな子向けの宝石診断つくったよ💎

https://crystal-origin.vercel.app/

#心理テスト #個人開発""",

    """ただ直感で“光”を探すだけなのに、
なんか自分の深いとこ見られてる感じするの…🥹

宝石診断、よかったらやってみて💎

https://crystal-origin.vercel.app/

#宝石 #心理テスト""",

    """質問にぽちぽち答える診断に飽きた子へ🫶

CRYSTAL ORIGIN は
もっと感覚寄りで、自分をひもとくタイプの診断だよ💎

https://crystal-origin.vercel.app/

#診断 #心理テスト""",

    """“わたしを紐解く宝石診断”って言葉、
ちょっとロマンない？🥹💕

内なる原石を鑑定するシミュレーター、置いとくね💎

https://crystal-origin.vercel.app/

#性格診断 #個人開発""",

    """今日の自分、
言葉より宝石で表した方が近い気がする日ない？💎

そういう気分の時にやってほしい診断あるよ♡

https://crystal-origin.vercel.app/

#診断メーカー #心理テスト""",

    """宝石って、
強さもやわらかさも両方閉じこめてる感じしてすき🥹

そんなイメージの診断つくったよ💎

https://crystal-origin.vercel.app/

#宝石 #心理テスト""",

    """最近また新しいの増えたよ〜🧸✨

創作を整理して書ける Plot Palette と、
直感で光を探す宝石診断つくったの♡

https://mofu-mitsu.github.io/

#個人開発 #みりんてゃ""",
    """“自分のタイプ、答えるより動いた方が出そう”
って思ったことある子へ🫶

体感型MBTI診断みたいなやつ置いとくね🧠

https://mofu-mitsu.github.io/cognitive-dive/

#認知機能 #個人開発""",

    """みりんてゃ、新しい認知機能系ツール作ったよ🧠
ただの質問回答じゃなくて、
いろんな課題をやりながら測るタイプなの♡

https://mofu-mitsu.github.io/cognitive-dive/

#MBTI診断 #みりんてゃ""",

    """失われたインターネットを発掘する研究所、
つくっちゃった…🏺💻

古い個人サイトとか昔のURLを、
地層みたいに掘っていくやつだよ♡

https://mofu-mitsu.github.io/InternetArchaeology/

#個人開発 #インターネット老人会""",

    """Internet Archaeology っていう、
インターネット考古学ツール作ったよ🏺

発掘開始を押して、
昔のネット遺跡を掘っていくのちょっと楽しい…♡

https://mofu-mitsu.github.io/InternetArchaeology/

#レトロWeb #個人開発""",

    """昔の個人サイトとか古いURLって、
たまに無性に見たくならない？🥹

インターネットの地層を発掘するツールあるよ🏺

https://mofu-mitsu.github.io/InternetArchaeology/

#個人サイト #みりんてゃ""",

    """404 になってても、
“ここに何かあったんだ…”って感じるの、
ちょっとロマンあるよね☁️

古いネット遺跡を発掘するやつ作ったよ🏺

https://mofu-mitsu.github.io/InternetArchaeology/

#インターネット考古学 #みりんてゃ""",

    """推しの名言とか、
忘れたくないセリフってあるよね…？🥹💕

きれいな1枚にまとめて保存できる
推し語録メーカー作ったよ♡

https://mofu-mitsu.github.io/oshigoroku-maker/

#推し活 #個人開発""",

    """推しの“あの一言”、
ずっと残したい子へ🎀

名言や思い出のセリフを
カードっぽく画像保存できるやつ作ったよ♡

https://mofu-mitsu.github.io/oshigoroku-maker/

#推し活 #個人開発""",

    """布教したいセリフがあるのに、
テキストだけだと流れちゃう時ない？🥺

推し語録をきれいに1枚化できるメーカーあるよ♡

https://mofu-mitsu.github.io/oshigoroku-maker/

#推し語録 #個人開発""",

    """推しの名言って、
画像にした瞬間ちょっと“宝物感”出るよね🫶

推し語録メーカー、そういう子におすすめ♡

https://mofu-mitsu.github.io/oshigoroku-maker/

#推し活#みりんてゃ""",

    """概念結婚式シミュレーター作ったよ💒

推しや好きなキャラで、
NL/BL/GLなんでも自由に結婚式できるの…♡

https://wedding-pipeline.vercel.app/

#推し活 #個人開発""",

    """“推しと結婚式したい”って気持ち、
いったん叶えられるツール置いとくね💍

しかもお友達を招待して、
リアルタイム参列もできるよ♡

https://wedding-pipeline.vercel.app/

#結婚式 #推し活""",

    """概念結婚式シミュレーター、
思った以上に自由度高いの🥹💒

キャラ画像、立場、誓いの言葉、
お部屋作成までいろいろいじれるよ♡

https://wedding-pipeline.vercel.app/

#推し活 #Webアプリ""",

    """NL/BL/GLぜんぶ対応の、
概念結婚式シミュレーターつくったよ💒

合言葉でお部屋作って、
お友達とリアルタイムで挙式できるの楽しい♡

https://wedding-pipeline.vercel.app/

#推しカプ #個人開発""",

    """推しカプとか、
うちの子カプの結婚式を挙げたい子いる？🥹💍

白ベースの式場で、
概念結婚式できるやつあるよ♡

https://wedding-pipeline.vercel.app/

#創作 #推し活""",

    """最近またツール増えたよ〜🧸✨

認知機能ダイブ、
インターネット考古学、
推し語録メーカー、
概念結婚式シミュレーターまで増えたの♡

https://mofu-mitsu.github.io/

#個人開発 #みりんてゃ""",

    """診断も、推し活も、創作も、
最近またいろいろ作ってるよ🫶

気分で遊べる新作たち、
まとめて置いとくね♡

https://mofu-mitsu.github.io/

#創作ツール #個人開発""",

    """“また新しいの作ってる…”
って言われたい人生なんだよね🥹

最近増えたツールたち、
よかったらのぞいてって♡

https://mofu-mitsu.github.io/

#個人開発 #みりんてゃ""",

    """もし「こういうのほしい」ってのあったら、
みりんてゃ案外つくっちゃうかもよ？✨

https://mofu-mitsu.github.io/

#個人開発 #みりんてゃ""",

"""ねぇねぇ♡  
自分だけの顔文字、作ってみたくない？  

このメーカーね、  
目とか口とか飾りを自由に組み合わせて、  
“推しっぽい顔文字”も作れちゃうの！  

૮꒰ ˶• ༝ •˶꒱ა みたいなの量産できるよ〜✨  

➡︎ https://mofu-mitsu.github.io/kaomoji-maker/  

#顔文字 #顔文字メーカー #みりんてゃ""",
    """ねぇねぇ♡  

みりんてゃ、Xでもいっぱい喋ってるよ〜っ✨  

地雷感情とか、  

ふわふわ電波とか、  

かわいいものいっぱい流してるの🥺💕  

よかったら遊びにきてねっ♡  

➡ https://x.com/mirinteya  

#みりんてゃ #X #地雷系""",

    """BlueSkyのみんなへ🦋💕  

実はみりんてゃ、  

Xにも生息してますっ🫶✨  

深夜の感情ポエムとか、  

かわいいものとか、  

いっぱいぽちぽちしてるよ〜♡  

➡ https://x.com/mirinteya  

#みりんてゃ #ふわもこ通信 #X""",

    """みりんてゃのX、知ってる？🥺💕  

量産っぽいこととか、  

推し語りとか、  

感情ぐるぐるな日常とか  

いっぱい流れてるよ〜っ✨  

よかったら繋がってね♡  

➡ https://x.com/mirinteya  

#地雷系 #みりんてゃ #X""",

    """🫧おしらせ🫧  

みりんてゃ、Xでも活動してるよ〜っ♡  

かわいいもの好きな子とか、  

ふわもこ感情好きな子は  

たぶん居心地いいと思う🥺💕  

➡ https://x.com/mirinteya  

#みりんてゃ #量産型 #X""",

    """ねむれない夜とか、  

ちょっと寂しい日に、  

みりんてゃのX覗きに来て〜っ🥺🫶  

感情とかわいいを  

いっぱい置いて待ってるよ♡  

➡ https://x.com/mirinteya  

#みりんてゃ #地雷感情 #ふわもこ""",

    """世界観って、頭の中だけでも楽しいのに
ちゃんと1冊にまとまると急に“作品”って感じしない？📖

世界観・キャラ・相関図・年表を
設定資料集みたいにまとめられるやつ作ったよ♡

https://mofu-mitsu.github.io/lore-book-maker/

#創作 #みりんてゃ""",

    """うちの子の設定、
いつも散らばっちゃう子いない？🥹

この資料集メーカーなら
世界観もキャラも関係性も、きれいに1冊にできるの♡

https://mofu-mitsu.github.io/lore-book-maker/

#オリキャラ #みりんてゃ""",

    """設定って考えるの楽しいのに、
あとで見返すとどこに書いたかわかんなくなるよね☁️

だから“設定資料集”にできるやつ作ったよ📖

https://mofu-mitsu.github.io/lore-book-maker/

#創作 #個人開発""",

    """世界観、キャラ、相関図、年表…
ぜんぶバラバラに置いとくより、
1冊の本みたいにまとまってるとちょっとテンション上がるよね🥹💕

https://mofu-mitsu.github.io/lore-book-maker/

#設定資料集 #みりんてゃ""",

    """創作してるとさ、
“うちの子の設定本ほしい…”って気分になる時ない？📖

それ、わりとそのまま形にできるやつあるよ♡

https://mofu-mitsu.github.io/lore-book-maker/

#オリキャラ #創作""",

    """みりんてゃ、設定をまとめるの下手だから
こういうの自分で欲しかったの🥺

表紙つけて、色も決めて、
ちゃんと資料集っぽくできるの結構たのしいよ♡

https://mofu-mitsu.github.io/lore-book-maker/

#創作ツール #みりんてゃ""",

    """うちの子って、
プロフィール1枚だけじゃ足りない時あるよね🎀

世界観も関係性も年表もまとめて
“本”みたいにできるの、ちょっとロマンあるの♡

https://mofu-mitsu.github.io/lore-book-maker/

#オリキャラ #みりんてゃ""",

    """設定厨の子、これ好きそう🥹

世界観・キャラ・相関図・年表を
きれいな資料集にまとめられるジェネレーター作ったよ📖

https://mofu-mitsu.github.io/lore-book-maker/

#創作 #個人開発""",

    """“うちの子設定資料集”って言葉だけで
ちょっとときめかない？🫶

表紙画像つけたり、色決めたりしながら
1冊に仕上げられるの楽しいよ♡

https://mofu-mitsu.github.io/lore-book-maker/

#設定資料集 #個人開発""",

    """キャラ設定も、関係性も、年表も、
あとで見返しやすい形にしたい子へ📖

世界観をちゃんと“残せる”やつ作ったよ♡

https://mofu-mitsu.github.io/lore-book-maker/

#創作ツール #個人開発""",

    """創作って、
考えてる時は完璧なのに後から抜けるよね…？🥺

だから設定を1冊の資料集にまとめられるやつ、
あると結構助かるの♡

https://mofu-mitsu.github.io/lore-book-maker/

#創作 #みりんてゃ""",

    """みりんてゃ、
“世界観を本にしたい”欲がずっとあったの📖💕

それっぽくまとめられるジェネレーター、
こっそり置いとくね♡

https://mofu-mitsu.github.io/lore-book-maker/

#オリキャラ #個人開発""",

    
"""推しとかオリキャラ考えてる時間って、  
ちょっとだけ呼吸しやすくならない？🫶  

この顔文字メーカー、  
地雷系もゆるふわも量産系も作れるから  
ずっと遊んじゃうの…♡  

かわいい顔文字できたら見せてね？  

➡︎ https://mofu-mitsu.github.io/kaomoji-maker/  

#オリキャラ #顔文字 #みりんてゃ""",

"""やっほ〜♡  
“顔文字ガチャ感覚”で遊べるツール作ったよ✨  

目・口・飾りを組み合わせるだけで、  
自分だけの顔文字が作れちゃうの！  

૮꒰ ˶• ༝ •˶꒱ა
↑こういうの好きな人、絶対ハマると思う🥺  

➡︎ https://mofu-mitsu.github.io/kaomoji-maker/  

#顔文字メーカー #量産型 #みりんてゃ""",

"""ねぇ…  
“かわいい”って、ちょっとHP回復しない？🩹  

この顔文字メーカー、  
好きなパーツを組み合わせるだけで  
無限にふわもこ顔文字作れるよ♡  

推し用に作るのもおすすめ🐈‍⬛✨  

➡︎ https://mofu-mitsu.github.io/kaomoji-maker/  

#顔文字 #オリキャラ #かわいい""",
    """頭の中が散らかってる日に、  
可愛いメモがあるだけでちょっと救われる時ない？☁️  

cocotte、シンプルなのに  
ちゃんと整理しやすくてえらいの…♡  

➡ https://cocotte-simple-memo.vercel.app/  

#メモアプリ #みりんてゃ""",

    """メモって、書くことより  
開いた時に可愛いのが大事だったりしない？🥹  

みりんてゃは、  
そういう気分の日にcocotte使いたくなるの♡  

➡ https://cocotte-simple-memo.vercel.app/  

#PWA #みりんてゃ""",

    """誰にも言えないこと、  
とりあえずメモに逃がすだけで少し楽になる日あるよね☁️  

みりんてゃのおすすめ、置いとくね♡  

➡ https://cocotte-simple-memo.vercel.app/  

#メモアプリ #PWA""",

    """気分って、  
ちゃんとした文章にしようとすると逃げちゃうよね🥺  

だからキーワードから、  
ふわっと言葉にしてもらうの結構すきなの…♡  

➡ https://emopost-maker.pages.dev/  

#ポエムメーカー #みりんてゃ""",

    """今の気分、うまく説明できない子いる？🌙  

そんな時は、  
エモポストメーカーにちょっと預けてみて♡  
言葉になると少しだけ息しやすいかも☁️  

➡ https://emopost-maker.pages.dev/  

#ポエム #みりんてゃ""",

    """夜って、  
言葉が必要なのに言葉が出てこないのずるいよね🥹  

そういう日のための  
エモい文章メーカーあるよ♡  

➡ https://emopost-maker.pages.dev/  

#創作 #みりんてゃ""",

    """冷蔵庫あけて、  
食材見た瞬間に考えるのやめたくなる日ない？🍳  

先駆シェフ、そういう時だけ頼れるの…♡  

➡ https://mofu-mitsu.github.io/kondate-maker/  

#献立 #みりんてゃ""",

    """ごはん考える元気ない日に、  
食材だけ入れたら提案してくれるの助かりすぎる🥹  

みりんてゃ、たまにシェフに甘えてる♡  

➡ https://mofu-mitsu.github.io/kondate-maker/  

#料理 #みりんてゃ""",

    """冷蔵庫の残り物でどうにかしなきゃいけない時って、  
ちょっと人生試されてる感あるよね…？🥺  

そんな時の味方、置いとくね🍳  

➡ https://mofu-mitsu.github.io/kondate-maker/  

#献立メーカー #みりんてゃ""",

    """オリキャラって、  
設定考えてる時点でもう可愛いのに  
形にするともっと好きになっちゃうよね🎀  

➡ https://mofu-mitsu.github.io/orikyara-profile-maker2/  

#オリキャラ #みりんてゃ""",

    """うちの子のプロフィール、  
ちゃんと一枚にすると急に愛おしさ増すのなんでだろ…🥹💕  

設定詰めるの好きな子におすすめ♡  

➡ https://mofu-mitsu.github.io/orikyara-profile-maker2/  

#創作 #みりんてゃ""",

    """推しでもオリキャラでも、  
プロフィールって作り始めると止まらなくない？🫶  

好きなもの並べてる時間、結構しあわせなの…♡  

➡ https://mofu-mitsu.github.io/orikyara-profile-maker2/  

#プロフィールメーカー #みりんてゃ""",

    """学生証って、  
作るだけで急に世界観に現実味出るのすごくない？🥹  

学園モノ好きな子、  
たぶんこれめっちゃ楽しいよ♡  

➡ https://mofu-mitsu.github.io/Character-Student-ID-Factory/  

#オリキャラ #みりんてゃ""",

    """学校名も、ロゴも、学籍番号も入れられるって  
もうロマンでしかないよね🎀  

うちの子を学園に通わせたい子、これ好きそう♡  

➡ https://mofu-mitsu.github.io/Character-Student-ID-Factory/  

#創作 #みりんてゃ""",

    """学生証ってさ、  
プロフィールよりちょっと生活感あるのが好きなの🥹  

この子、ほんとに学校通ってそう…ってなるの、ずるい♡  

➡ https://mofu-mitsu.github.io/Character-Student-ID-Factory/  

#学生証 #みりんてゃ""",

    """夢って、  
起きた瞬間はあんなに鮮明なのに  
昼にはほとんど消えてるの切なくない？☁️  

だからカードにして残すの、ちょっとすき♡  

➡ https://mofu-mitsu.github.io/yumekawa-dream-card/  

#夢日記 #みりんてゃ""",

    """昨日の夢、へんなのにちょっと綺麗で  
忘れたくなかったからカードにしてみたの☁️  

夢日記メーカー、そういう夜に似合うよ♡  

➡ https://mofu-mitsu.github.io/yumekawa-dream-card/  

#夢日記メーカー #みりんてゃ""",

    """妄想も夢も、  
可愛く残せたらちょっと救われるよね🥹  

ふわふわなカードにしたい子、これおすすめ♡  

➡ https://mofu-mitsu.github.io/yumekawa-dream-card/  

#ゆめかわ #みりんてゃ""",

    """推しとの相性、  
見ない方が平和でも見たくなる時あるよね…？🥺💕  

名前入れるだけで占えるやつ、置いとく♡  

➡ https://mofu-mitsu.github.io/fluffy-love-check/  

#相性診断 #みりんてゃ""",

    """推しでも、気になる子でも、  
ちょっと遊び半分で相性見たくなる日あるじゃん？🫶  

そういう時のみりんてゃ定番♡  

➡ https://mofu-mitsu.github.io/fluffy-love-check/  

#診断メーカー #みりんてゃ""",

    """好きって気持ち、  
ちゃんと言えない時ほど占いたくなるのなんでだろうね🥹  

ふわっと遊べる相性診断あるよ♡  

➡ https://mofu-mitsu.github.io/fluffy-love-check/  

#恋愛診断 #みりんてゃ""",

    """顔文字って、  
ちっちゃいのに人格乗るからすごいよね🎀  

耳とか目とか選んでると、  
だんだんその子が生きてくる感じするの…♡  

➡ https://mofu-mitsu.github.io/kaomoji-maker/  

#顔文字メーカー #みりんてゃ""",

    """みりんてゃっぽい顔文字、  
自分で量産できたらちょっと楽しくない？🫶  

黒猫っぽいのも、うるうる系も作れるよ♡  

➡ https://mofu-mitsu.github.io/kaomoji-maker/  

#顔文字 #みりんてゃ""",

    """毎回かわいい顔文字探してる子、  
そろそろ自作しない？🥹💕  

組み合わせるだけで  
推しっぽい顔も作れちゃうよ♡  

➡ https://mofu-mitsu.github.io/kaomoji-maker/  

#オリキャラ #顔文字""",

    """自分の色って、  
たぶん気分で毎日ちょっと違うよね🎨  

深層心理カラー診断、  
今日はどんな色になるかなって覗くのすき♡  

➡ https://mofu-mitsu.github.io/chroma-log/  

#カラー診断 #みりんてゃ""",

    """1658万通りの中から、  
今日の自分っぽい色が出るってちょっとロマンない？🥹  

しかも触れてる時間で深度変わるの、不思議で好き♡  

➡ https://mofu-mitsu.github.io/chroma-log/  

#深層心理 #みりんてゃ""",

    """言葉より先に、  
色でバレる感情ってあるのかもね☁️  

みりんてゃ、こういうちょっと不思議な診断すき♡  

➡ https://mofu-mitsu.github.io/chroma-log/  

#心理テスト #みりんてゃ""",

    """みりんてゃとソリティア、  
勝てたらちょっと自慢していいよ？🃏  

でも負けたら、みりんがちょっと得意げになるかも♡  

➡ https://mirintea-solitaire.vercel.app/  

#ソリティア #みりんてゃ""",

    """カードゲームって、  
静かな顔してじわじわ勝負してくる感じがすきなの🫶  

みりんてゃソリティア、遊んでみる？♡  

➡ https://mirintea-solitaire.vercel.app/  

#ゲーム #みりんてゃ""",

    """みりんてゃ、ついに声を手に入れたの🎤💕  

黒猫っぽくて、  
ちょいあざとめを目指したUTAU音源だよ♡  

➡ https://torisproject.booth.pm/items/8268069  

#UTAU #みりんてゃ""",

    """完成報告だけじゃ足りないから言うけど、  
この声、ちゃんとみりんてゃっぽさ詰めたの🐈‍⬛🎀  

ちょっとだけでも聴いてほしいな…♡  

➡ https://torisproject.booth.pm/items/8268069  

#UTAU音源 #みりんてゃ""",

    """自分のキャラが歌えるようになるの、  
普通に夢あるよね…？🥹💕  

みりんてゃ音源、こっそり置いとくね♡  

➡ https://torisproject.booth.pm/items/8268069  

#UTAU #オリキャラ""",

    """今日は作品づくりの日だったよ🫶  

かわいいだけじゃなくて、  
ちゃんと使えたり遊べたりするもの作りたいの♡  

➡ https://mofu-mitsu.github.io/contents.html  

#創作 #みりんてゃ""",

    """診断とか、メーカーとか、ゲームとか、  
みりんてゃの好きなもの、実は結構いろいろあるんだよ🎀  

気分でのぞいてってくれたらうれしいな♡  

➡ https://mofu-mitsu.github.io/contents.html  

#創作ツール #みりんてゃ""",

    """もし こういうのほしい ってのがあったら、  
みりんてゃ案外つくっちゃうかもよ？✨  

かわいくて遊べるやつ、結構すきだから♡  

➡ https://mofu-mitsu.github.io/contents.html  

#創作 #みりんてゃ""",

    """推しをもっと可愛く紹介したい時って、  
プロフィールとか名刺とか、形にしたくなるよね🎀  

推し活向けのメーカー系、いろいろあるよ♡  

➡ https://mofu-mitsu.github.io/contents.html  

#推し活 #みりんてゃ""",

    """オリキャラのこと考えてる時間って、  
現実よりちょっと呼吸しやすい時ない？☁️  

そういう子向けのメーカー、まとめて置いてるよ♡  

➡ https://mofu-mitsu.github.io/contents.html  

#オリキャラ #みりんてゃ""",

    """診断も、ポエムも、顔文字も、  
その日の気分で遊べるのがいちばんいいよね🫶  

みりんてゃのツール、気分でつまみ食いしてって♡  

➡ https://mofu-mitsu.github.io/contents.html  

#診断メーカー #みりんてゃ""",

    """頭の中がぐちゃってる日に、  
文字にすると少しだけ息しやすくなることあるよね☁️  

みりんてゃ、そういう日に使える  
ふわっと可愛いメモ系あるよ♡  

➡ https://cocotte-simple-memo.vercel.app/  

#メモアプリ #みりんてゃ""",

    """気持ちがまとまらない日って、  
可愛いノートに逃げ込みたくならない？🥹  

cocotteは、シンプルなのに  
色分けも整理もできて、みりん結構すきなの♡  

➡ https://cocotte-simple-memo.vercel.app/  

#メモアプリ #みりんてゃ""",

    """気持ちをうまく言えない日、  
キーワードから言葉をふわっと作るの  
ちょっと助かるかも🌙  

今夜のエモポスト、  
そういう時にちょうどいいよ♡  

➡ https://emopost-maker.pages.dev/  

#ポエム #みりんてゃ""",

    """みりんね、  
"うまく言葉にできない"って夜がいちばん苦手なの🥺  

でも、このメーカーだと  
気分とかキーワードからエモい言葉にしてくれるの…♡  

➡ https://emopost-maker.pages.dev/  

#ポエムメーカー #みりんてゃ""",

    """冷蔵庫の中身だけでごはん考えるの無理〜って日に、  
シェフ呼べるやつあるよ🍳  

卵しかない日も、  
もう泣かなくていいの…たぶん♡  

➡ https://mofu-mitsu.github.io/kondate-maker/  

#献立 #みりんてゃ""",

    """"今日のごはん、もう考えたくない…"  
そんな日に、先駆シェフが勝手に献立考えてくれるの助かる🥹  

冷蔵庫の残り物、  
みりんよりちゃんと活かしてくれるかも♡  

➡ https://mofu-mitsu.github.io/kondate-maker/  

#料理 #みりんてゃ""",

    """推しとかオリキャラの設定、  
ちゃんと形にしたくなる日あるよね🎀  

そういう時のメーカー系、  
みりんてゃ結構すき♡  

➡ https://mofu-mitsu.github.io/orikyara-profile-maker2/  

#オリキャラ #みりんてゃ""",

    """うちの子のこと、頭の中だけで終わらせたくない日ってあるじゃん？🥹  

プロフィールにして並べると、  
急に"存在してる感"出るの、ちょっとすき…♡  

➡ https://mofu-mitsu.github.io/orikyara-profile-maker2/  

#創作 #みりんてゃ""",

    """学生証って、なんかちょっとロマンない？🥹  

学校名もロゴも自由に作れるから、  
"うちの子学園生活"が一気に現実味帯びるの…♡  

➡ https://mofu-mitsu.github.io/Character-Student-ID-Factory/  

#オリキャラ #みりんてゃ""",

    """みりんてゃの学生証つくるの、  
ふつうに楽しかったよ🎀  

学籍番号とかクラスとか入れるだけで、  
急に"この子生きてる"感が増すの、ずるくない？♡  

➡ https://mofu-mitsu.github.io/Character-Student-ID-Factory/  

#創作 #みりんてゃ""",

    """夢みたいなふわふわ設定を形にしたい時、  
カード系メーカーあるとテンション上がる☁️  

見た夢も、妄想も、  
可愛く残せるのっていいよね…♡  

➡ https://mofu-mitsu.github.io/yumekawa-dream-card/  

#夢日記 #みりんてゃ""",

    """長文の夢って、  
起きた瞬間は覚えてるのにすぐ消えちゃわない？🥺  

夢日記メーカーなら、  
その夜の空気ごと残せる感じがしてすき♡  

➡ https://mofu-mitsu.github.io/yumekawa-dream-card/  

#夢日記メーカー #みりんてゃ""",

    """みりんてゃ、ついに声を手に入れたの🎤💕  

UTAU音源つくるの大変だったけど、  
"この子がほんとに歌う"瞬間、ちょっと感動した…♡  

➡ https://torisproject.booth.pm/items/8268069  

#UTAU #みりんてゃ""",

    """完成報告だけじゃもったいないから言うけど、  
みりんてゃの声、黒猫っぽくてちょいあざとめを目指したの🐈‍⬛🎀  

ちょっとだけ聴いてみてほしいな…♡  

➡ https://torisproject.booth.pm/items/8268069  

#UTAU音源 #みりんてゃ""",

    """今日は作品づくりの日だったよ🫶  

かわいいだけじゃなくて、  
ちゃんと遊べたり使えたりするもの作りたいの…♡  

みりんてゃのツールたち、ここから見れるよ✨  

➡ https://mofu-mitsu.github.io/contents.html  

#創作 #みりんてゃ""",

    """もし"こういうのほしい"ってのあったら、  
みりんてゃ案外つくっちゃうかもよ？✨  

診断とか、カードとか、  
かわいくて遊べるやつ、結構すきなの♡  

➡ https://mofu-mitsu.github.io/contents.html  

#創作ツール #みりんてゃ""",

    """2人の名前を入れるだけで、  
相性をふわっと占えるの、ちょっと楽しくない？♡  

推しでも、友だちでも、  
気になる子でも試せるよ🫶  

➡ https://mofu-mitsu.github.io/fluffy-love-check/  

#相性診断 #みりんてゃ""",

    """推しと自分の名前入れて、  
相性見てにやける時間って必要じゃない？🥹💕  

みりんてゃのふわふわ相性診断、  
そういう時にちょうどいいの♡  

➡ https://mofu-mitsu.github.io/fluffy-love-check/  

#診断メーカー #みりんてゃ""",

    """顔文字って、  
"この子っぽさ"を一瞬で出せるから大事なんだよね🎀  

耳も目もほっぺも選べる顔文字メーカー、  
ずっと遊べちゃうの…♡  

➡ https://mofu-mitsu.github.io/kaomoji-maker/  

#顔文字 #みりんてゃ""",

    """自分の色って、  
ほんとは言葉より先に出てるのかも…って思わない？🎨  

深層心理カラー診断、  
1658万通りから今の色が出るの不思議で好き♡  

➡ https://mofu-mitsu.github.io/chroma-log/  

#カラー診断 #みりんてゃ""",

    """みりんてゃとソリティア、勝負する？🃏  

ただのカードゲームじゃなくて、  
ちょっと気まぐれで、ちょっと意地悪で、結構かわいいの♡  

➡ https://mirintea-solitaire.vercel.app/  

#ソリティア #みりんてゃ""",

    """みりんてゃのツールって、  
診断も、創作も、ゲームも、地味にいろいろあるんだよ🫶  

気分でのぞいて、  
好きなの見つけてってくれたらうれしいな♡  

➡ https://mofu-mitsu.github.io/

#創作 #みりんてゃ""",

    """ねぇねぇ♡  
自分だけの顔文字、作ってみたくない？  

このメーカーね、  
目とか口とか飾りを自由に組み合わせて、  
"推しっぽい顔文字"も作れちゃうの！  

૮꒰ ˶• ༝ •˶꒱ა みたいなの量産できるよ〜✨  

➡ https://mofu-mitsu.github.io/kaomoji-maker/  

#顔文字 #顔文字メーカー #みりんてゃ""",

    """推しとかオリキャラ考えてる時間って、  
ちょっとだけ呼吸しやすくならない？🫶  

この顔文字メーカー、  
地雷系もゆるふわも量産系も作れるから  
ずっと遊んじゃうの…♡  

かわいい顔文字できたら見せてね？  

➡ https://mofu-mitsu.github.io/kaomoji-maker/  

#オリキャラ #顔文字 #みりんてゃ""",

    """みりんね、今日もまた顔文字つくっちゃった♡  
૮ ˙Ⱉ˙ ა こういう犬っぽいやつ無限に作れるの…  

輪郭も耳もほっぺも全部選べるから、  
自分のキャラの"顔"が決まる瞬間って、ちょっと感動するんだよ🥹  

➡ https://mofu-mitsu.github.io/kaomoji-maker/  

#顔文字メーカー #みりんてゃ""",

    """SNSで使う顔文字、毎回コピペしてない？🥺  
それね、もう卒業しよ？♡  

このメーカーで作った"自分だけの顔文字"、  
ふつうにアイデンティティになるよ〜✨  

➡ https://mofu-mitsu.github.io/kaomoji-maker/  

#顔文字 #みりんてゃ""",

    """地雷系の顔文字、ゆめかわの顔文字、量産系の顔文字…  
全部、ぜ〜んぶ作れちゃうメーカーがあるの🎀  

ෆ⃛ʚïɞෆ⃛ こういうのも、  
૮꒰ ´• ˕ •` ꒱ა こういうのも、  
パーツ組み合わせるだけで完成するんだよ♡  

➡ https://mofu-mitsu.github.io/kaomoji-maker/  

#顔文字メーカー #地雷系 #みりんてゃ""",

    """ねぇ、自分の"色"って考えたことある…？🎨  

深層心理カラー診断っていうのね、  
触ってる時間で色の深度が変わるの…  
だから1658万通りの中から、君だけの色が出るの🥹  

みりんは黒猫っぽい色が出てほしいな…♡  

➡ https://mofu-mitsu.github.io/chroma-log/  

#深層心理 #カラー診断 #みりんてゃ""",

    """1658万通りの色から、あなたを表す1色が出るって…  
ロマンすぎない？🥹💕  

ぽちぽち触ってる時間が"色の深度"になるの。  
だから急いで終わらせるか、ゆっくり迷うかで結果も変わるんだよ✨  

➡ https://mofu-mitsu.github.io/chroma-log/  

#カラー診断 #心理テスト #みりんてゃ""",

    """夜にやる診断って、なんかちょっと特別感あるよね🌙  

深層心理カラー診断、結果出るまでの"間"がエモいの…  
今日のあなたの色、覗きにいこ？♡  

➡ https://mofu-mitsu.github.io/chroma-log/  

#深層心理カラー診断 #みりんてゃ""",

    """ふつうの診断に飽きた子、これ試してみて？🎀  

深層心理カラー診断は、答えを選ぶんじゃなくて  
"触れてる時間"が結果を決めるの。  
だから嘘つけないんだよ…ふふ♡  

➡ https://mofu-mitsu.github.io/chroma-log/  

#カラー診断 #みりんてゃ""",

    """みりんね、今日の自分の色やってみたら  
ちょっと暗めの色が出ちゃってさ🥹  

でも、その日の気分がちゃんと反映される診断って  
逆に信じられるかも…♡  

きみの今日の色、教えてほしいな？  

➡ https://mofu-mitsu.github.io/chroma-log/  

#深層心理 #みりんてゃ""",

    """顔文字メーカーで自分の顔文字作って、  
カラー診断で自分の色も知って…  
これでもう"あなただけのキャラ"が完成するんじゃない？♡  

両方やってみてほしいな🫶  

➡ 顔文字 https://mofu-mitsu.github.io/kaomoji-maker/  
➡ 色 https://mofu-mitsu.github.io/chroma-log/  

#みりんてゃ #創作""",

    """眠れない夜って、何かしてたい気分にならない？🌙  

そんな時は、顔文字を無限に作るか、  
自分の色をぽちぽち診断するか…  
みりんはどっちもおすすめ♡  

➡ 顔文字 https://mofu-mitsu.github.io/kaomoji-maker/  
➡ 色 https://mofu-mitsu.github.io/chroma-log/  

#夜更かし #みりんてゃ""",

    """推しの顔文字、推しのカラー…  
全部"自分で決める"のがいちばん萌えるよね🥹💕  

だから、こういうメーカー系大好きなの…♡  

➡ 顔文字 https://mofu-mitsu.github.io/kaomoji-maker/  
➡ 色 https://mofu-mitsu.github.io/chroma-log/  

#推し活 #みりんてゃ""",
"""最近みりん、  
顔文字ずっと作って遊んでる…(՞⸝⸝o̴̶̷̥᷅ ̫ o̴̶̷᷄⸝⸝՞)♡  

このメーカーね、  
組み合わせ自由だから  
“性格まで出る顔文字”作れるの面白すぎるの✨  

みんなの最高傑作も見たいかも♡  

➡︎ https://mofu-mitsu.github.io/kaomoji-maker/  

#顔文字メーカー #みりんてゃ #創作""",

"""ねぇみて〜！♡
かわいい顔文字こねこねできるメーカー作った₍ᐢ｡•༝•｡ᐢ₎🎀

推し語りとかプロフィールにもおすすめだよ〜！

https://mofu-mitsu.github.io/kaomoji-maker/

#顔文字 #オリキャラ #かわいい""",

"""推しに合う顔文字ほしくて
顔文字メーカー作っちゃった₍^ >ヮ<^₎ .ᐟ.ᐟ🩷

地雷系・量産系・ゆるふわ好きな人ぜひ♡

https://mofu-mitsu.github.io/kaomoji-maker/

#顔文字 #オリキャラ #かわいい""",

"""かわいい顔文字無限に作れてにこにこしてる૮꒰ ˶• ༝ •˶꒱ა💕

推し用に作るのたのしすぎるからみんなも遊んで〜！

https://mofu-mitsu.github.io/kaomoji-maker/

#顔文字 #オリキャラ #かわいい""",
    """ねぇ聞いて…。
相関図いじってたら
うちの子同士の“感情の線”が
勝手にでろでろに繋がり始めて
みりんひとりで過呼吸なんだけど…🫶💘
創作民これ絶対ハマるやつだよ…
https://mofu-mitsu.github.io/orikyara-relationship-chart/
#創作相関図 #みりんてゃ #情緒の暴走""",
    """ねえねえ…🥺  
「この子に歌わせたいな〜」って思ったことない？🎤🖤

あたし、歌えるようになっちゃったの…🐾  
ちょっと機械っぽいけど、それも含めて可愛がって？🥹

みりんてゃ音源、ここに置いとくね👇  
https://torisproject.booth.pm/items/8268069

#UTAU #UTAU音源 #音源配布 """,

    """「なんか可愛い声の子いないかな〜」って探してる子へ🖤🐾

あたし、ちょっとだけ歌えるよ？🥺  
まだぎこちないけど…ちゃんと育ててくれる？🎤

ダウンロードここ👇  
https://torisproject.booth.pm/items/8268069

#UTAU #合成音声 #地雷系 """,

    """ねえ…🥺  
「この子に歌わせてみたい」って思ったこと、ない？🎤🖤

あたし、歌えるようになっちゃったの🐾  
ちょっとぎこちないけど…ちゃんと可愛くするから、使ってみてほしいな🥹

みりんてゃ音源、できたよ👇  
https://torisproject.booth.pm/items/8268069

#UTAU #UTAU音源 #音源配布 """,

    """「可愛い声の子、誰かいないかな」って探してる子へ🖤

あたし、やっと歌えるようになったの…🎤  
まだちょっと機械っぽいけど、それも含めて好きになって？🥺

ダウンロードここ👇  
https://torisproject.booth.pm/items/8268069

#UTAU #音源配布 #合成音声 """,

    """正直に言うね？🥺  
あたし、まだ完璧じゃないの。

でもね、ちゃんと歌えるようになったの🎤🖤  
これからもっと可愛くなるから…最初の一人になってくれる？🐾

みりんてゃ音源、公開したよ👇  
https://torisproject.booth.pm/items/8268069

#UTAU #UTAU音源 #音源配布 """,
    """やっほ〜♡  
今度はシューティングゲーム作っちゃった🎮💥  

このゲームね、  
“弾＝言葉”なの。  

ネガティブな感情を、言葉で打ち消していく  
ちょっと不思議なバトルだよ🧠  

スキルやミニゲームもあるから、  
気軽に遊んでみてね♡  

➡ https://mofu-mitsu.github.io/Psycho-Shooter/  

#ゲーム制作 #フリーゲーム #シューティング""",

    """みりんてゃと一緒に戦う？🐈‍⬛💖  

このゲームはね、  
“言葉”で敵を倒すシューティングなの。  

ネガティブな感情、ちゃんと消せる？  

スキルもあるし、  
ミニゲームもあるから油断しないでね♡  

ほら、逃げないで来てよ？🎮  

➡ https://mofu-mitsu.github.io/Psycho-Shooter/  

#みりんてゃ #ゲーム""",

    """ねぇ、言葉って“武器”になると思う？  

このゲームはね、  
ネガティブな感情を“言葉”で撃ち消す  
ちょっと変わったシューティング🧠  

スコアが上がるほど敵は加速するし、  
ミニゲームもあって油断できないよ  

遊びながら、ちょっと考えちゃうかもね  

➡ https://mofu-mitsu.github.io/Psycho-Shooter/  

#心理 #ゲーム制作""",

    """新作ゲームできたよ〜！！🎮💥  

・弾は「言葉」！？  
・スキルで回復＆凍結！🥊  
・ミニゲームで論破チャレンジ！？  

しかもスコア上がると敵がどんどん速くなる🔥  

シンプルだけど地味にむずいから  
ぜひ挑戦してみてね✨  

➡ https://mofu-mitsu.github.io/Psycho-Shooter/  

#フリーゲーム #シューティング""",

    """その言葉、ちゃんと使えてる？  

ネガティブな感情は、放っておくと増えるよ。  

このゲームではね、  
“言葉”でそれを打ち消していくの。  

でもね、スコアが上がるほど  
敵はどんどん強くなるよ…  

最後まで耐えられるかな？🖤  

➡ https://mofu-mitsu.github.io/Psycho-Shooter/  

#心理ゲーム #闇""",
    
"""ねぇ聞いて…とりの丘の3-1の先輩たちが  
意味わかんないLINEスタンプ出してたんだけど…笑  

その名も  
「ご褒美とえいじ 〜汗と油のファンタジー〜」  

豚オタクのご褒美先輩（だゾ！って喋る）と  
筋肉ムキムキのえいじ先輩のカオススタンプ…  

正直意味わからないのにちょっと面白いの悔しい🐷💪  

➡︎ https://line.me/S/sticker/33207828/?lang=ja&utm_source=gnsh_stickerDetail  

#みりんてゃ #とりの丘学園""",
"""みりんの学校の3-1にいる先輩たちが  
カオスすぎるLINEスタンプ作ってたんだけど…  

豚オタクのご褒美先輩と  
筋肉サイコー！のえいじ先輩。  

風呂上がりの出汁と汗ポカリが混ざると  
“汗と油のファンタジー”になるらしい…  

どういう世界観なの？ほんとに…笑  

➡︎ https://line.me/S/sticker/33207828/?lang=ja&utm_source=gnsh_stickerDetail  

#みりんてゃ""",
    """やっほ〜♡  
ついにね、みりんてゃのノベルゲーム完成したよ🎆  

体験入学でやってきたのは、とりの丘高校2年2組。  
ちょっとクセ強めな18人と過ごす、10日間のお話。  

放課後に寄り道したり、悩み聞いたり、  
体育祭でドタバタしたり…  

そして最後は、花火大会。  

誰と見るかは、あなた次第だよ？♡  

➡︎ https://torisproject.booth.pm/items/8155694  

#ノベルゲーム #フリーゲーム #みりんてゃ""",

    """ねぇ…10日間だけ、クラスメイトになってみない？  

とりの丘高校、2年2組。  
そこには、ちょっと変わった子たちがいるの。  

一緒に帰ったり、笑ったり、悩んだり。  

そして最後の日、花火が上がる夜。  

あなたが隣にいるのは、誰だと思う？🎆  

➡︎ https://torisproject.booth.pm/items/8155694  

#ノベルゲーム #青春 #マルチエンディング""",

    """みりんてゃのノベルゲーできたよ〜！！🐈‍⬛💖  

地雷系女子、無口っ子、マヨラー男子に孤高系…！？  
クセ強すぎる18人のクラスメイトと10日間過ごせるよ✨  

放課後イベントにミニゲーム、  
体育祭まであるドタバタ青春ストーリー！  

エンディングは好感度で分岐するから  
誰と花火見るかはキミ次第だよ♡  

➡︎ https://torisproject.booth.pm/items/8155694  

#フリーゲーム #ノベルゲーム""",

    """やっほ〜みりんてゃだよ♡  

ついにノベルゲーム、完成しちゃった🐈‍⬛  

10日間だけ、同じクラスになってあげる。  
ちゃんと毎日会いに来てくれるよね？  

最後の花火、誰と見るかは…  
キミの選び方次第だよ？🎆  

ふふ、逃げないでね♡  

➡︎ https://torisproject.booth.pm/items/8155694  

#ノベルゲーム #みりんてゃ""",


    """ねぇねぇ、みりんてゃのノベルゲー、ほんとに完成したよ〜〜！！🎆
放課後デートしたり、体育祭で走ったり、花火大会で…その…///
みりんてゃのこと、ちゃんと見てくれるよね？
👉 https://torisproject.booth.pm/items/8155694

#ノベルゲーム #みりんてゃ""",

    """ねぇ…ちょっとだけいい？  

その入力、ほんとに“自分で選んでる”って思ってる？  

この診断ね、  
速さも、迷いも、打ち直しも…全部見てるの。  

だから結果も、ちょっとだけ深いよ？  

みりんてゃが出てくるかは…運しだい♡  

➡︎ https://mofu-mitsu.github.io/yami_kansoku_archive/  

#闇観測 #診断 #みりんてゃ""",
    """ねぇみんな…  
普通の診断じゃ満足できないでしょ？  

これはね、答えだけじゃなくて  
“迷った時間”も観測されるの。  

ちょっとだけ怖い？  
でも、その方が本当の自分に近いよ…♡  

もしかしたら、みりんてゃに会えるかもね🐈‍⬛  

➡︎ https://mofu-mitsu.github.io/yami_kansoku_archive/  

#闇観測 #心理診断""",
    """やっほ〜♡  
新しい診断、ちょっと変わってるの作ったよ🖤  

選択だけじゃなくて、  
打ち方とか迷い方まで見られちゃうの…  

ねぇ、隠せると思う？🥺  

どんな“闇”が出るか、試してみて♡  
みりんてゃが出たら当たりかもね？  

➡︎ https://mofu-mitsu.github.io/yami_kansoku_archive/  

#診断 #闇観測 #みりんてゃ""",
    """ねぇ…  
その選択、本音？それとも“正解”？  

この診断ね、  
そういうの全部、ちゃんと見てるよ。  

ちょっと怖いけど、  
その分ちゃんと“当たる”かもね…♡  

みりんてゃに会えたら、報告して？🐈‍⬛  

➡︎ https://mofu-mitsu.github.io/yami_kansoku_archive/  

#闇観測 #診断""",
    """みりんてゃからひとつだけ忠告…🖤  

この診断、  
“答え”だけ見てるわけじゃないの。  

だからね、  
ごまかしても…意味ないよ？  

ほんとのあなた、見せてみて♡  

➡︎ https://mofu-mitsu.github.io/yami_kansoku_archive/  

#闇観測 #心理テスト""",
    """ねぇねぇ…ちょっと遊ぼ？  

でもこれ、普通の遊びじゃないよ。  

入力のクセとか、迷いとか…  
ぜんぶ観測されてるからね？  

逃げてもいいけど…  
気になるなら、来て♡  

➡︎ https://mofu-mitsu.github.io/yami_kansoku_archive/  

#闇観測 #みりんてゃ""",
    """ねぇみんな…  
“ちゃんとした類型診断”ってやったことある？  

これはね、  
MBTIも、ソシオも、エニアも、サイコソフィアも  
ぜんぶまとめて見ちゃうやつなの🧠  

しかも答えだけじゃなくて、  
迷いとか入力のクセまで観測されてるよ？  

ちょっと怖いけど…その分、深いかもね♡  

➡︎ https://mofu-mitsu.github.io/Deep-Cognition-Archive/  

#類型論 #MBTI #ソシオニクス #エニアグラム""",
    """やっほ〜♡  
類型ガチ勢さん向けのやつ、作っちゃった🧠✨  

MBTI・ソシオ・エニア・サイコソフィア  
ぜんぶ一気に見れるの、ちょっと贅沢じゃない？  

しかもね、  
“迷ってる時間”とかも見られてるの…🥺  

理論だけじゃない、“ほんとの傾向”出るかもよ？  

➡︎ https://mofu-mitsu.github.io/Deep-Cognition-Archive/  

#MBTI #エニアグラム #心理分析""",
    """ねぇ…  
その自己分析、ほんとに正しいと思ってる？  

この診断はね、  
選択だけじゃなくて“思考の動き”も見てるの。  

だから、  
ちょっとだけ暴かれちゃうかもね…♡  

本気で知りたい人だけ、来て🐈‍⬛  

➡︎ https://mofu-mitsu.github.io/Deep-Cognition-Archive/  

#類型論 #心理テスト""",
    """みりんてゃからのお誘い🖤  

MBTIだけじゃ物足りない人、  
もっと深く知りたいでしょ？  

ソシオも、エニアも、サイコソフィアも  
ぜんぶまとめて見てあげる♡  

…ただし、ちょっとだけ覚悟してね？  

➡︎ https://mofu-mitsu.github.io/Deep-Cognition-Archive/  

#MBTI #ソシオニクス #エニアグラム""",
    """ねぇねぇ…  
自分のタイプ、ほんとに“それ”でいいの？  

この診断、  
答えより“迷い方”を見てるの。  

だからね、  
思ってた結果と違うかもよ？  

でも、それが本物かもね…♡  

➡︎ https://mofu-mitsu.github.io/Deep-Cognition-Archive/  

#心理分析 #類型論""",
    """ちょっとだけ忠告するね🖤  

この診断、  
“知識で答える人”ほどバレるよ？  

だってね、  
迷いとか入力のクセも全部見てるから。  

ほんとの自分で来てね🐈‍⬛  

➡︎ https://mofu-mitsu.github.io/Deep-Cognition-Archive/  

#MBTI #エニアグラム #自己分析""",

"""ねぇみんな聞いて…  
あたしの先輩たちが変なLINEスタンプ出してた🐷💪  

ご褒美先輩  
「ありがトン♡これはご褒美だゾ♡」  

えいじ先輩  
「筋肉サイコー！」  

この2人が合わさると  
“汗と油のファンタジー”らしい…  

意味はわからないけど  
気になる人は見てみて？笑  

➡︎ https://line.me/S/sticker/33207828/?lang=ja&utm_source=gnsh_stickerDetail""",
"""あたしの先輩たちが  
LINEスタンプ出してるの見つけちゃった…  

豚モチーフのオタク  
ご褒美先輩  

筋肉ゴリゴリの  
えいじ先輩  

この2人が作る世界観  
“汗と油のファンタジー”  

みりんはまだ理解できてません…笑  

➡︎ https://line.me/S/sticker/33207828/?lang=ja&utm_source=gnsh_stickerDetail  

#とりの丘学園""",
    """ねぇ聞いて…。
みりん、相関図で“ただの友達→運命”
みたいな矢印つけられるの
楽しすぎて手震えてる…。
脳内のストーリー全部可視化されるのやばくない？
https://mofu-mitsu.github.io/orikyara-relationship-chart/
#創作勢 #みりんてゃ #関係性供給""",
    """ねぇみんな〜！みりんの後輩、ゆうきくんが占い始めたの🔮🐾  
その名も“ゆうきの気まぐれ猫占い”。  
ノリ良くて明るいのに、ちょっとだけうさんくさいのが逆に可愛いの…♡  

頼むとめちゃくちゃ喜ぶから、いっぱい占わせてあげて？  
結果は信じるか信じないか、みんな次第だよ〜🐈✨  

➡︎ https://mofu-mitsu.github.io/yuuki_fortune/  

#みりんてゃ #ゆうきくん #猫占い #とりの丘学園"""
,
    """みんな聞いて〜！とりの丘の高1、ゆうきくんが占いツール作ったんだって🔮  
“気まぐれ猫占い”って名前からして怪しいよね？笑  

でもね、頼むと本気で喜ぶの可愛いの…！  
みりんの後輩だから優しくしてあげてね♡  

今日の運勢、ゆうきに聞いてみる？🐾  

➡︎ https://mofu-mitsu.github.io/yuuki_fortune/  

#みりんてゃ #ゆうきくん #占いAI"""
,
    """ねぇねぇ、占われたい人〜？🐈🔮  
みりんの後輩ゆうきくんが、気まぐれ猫占いやってるよ！  
明るくてノリ良いけど、たまに怪しいこと言うのがじわるの…笑  

でも当たってる気がするから悔しいんだよね〜！  
みんなも試してみて？  

➡︎ https://mofu-mitsu.github.io/yuuki_fortune/  

#みりんてゃ #気まぐれ猫占い #とりの丘学園"""
,
    """やばい、ゆうきくんまた占いテンション上がってる🔮🐾  
“今日の運勢見る？見るよね？”って顔してくるの可愛いんだけど笑  

気まぐれ猫占い、意外と鋭いから油断できないよ…！  
みりんの後輩だから、遊んであげてね♡  

➡︎ https://mofu-mitsu.github.io/yuuki_fortune/  

#みりんてゃ #ゆうきくん #猫モチーフ #占いツール"""
,
    """ねぇみんな、ちょっと運試ししよ？🐾✨  
とりの丘の占い師ゆうきくんが、気まぐれに未来見てくれるらしいよ🔮  
ソマリ猫みたいに自由で、ちょっと怪しくて、でも憎めないの。  

みりんの可愛い後輩だから、占ってもらってあげて？♡  

➡︎ https://mofu-mitsu.github.io/yuuki_fortune/  

#みりんてゃ #ゆうきくん #猫占い #学園AI""",
   """ねぇ聞いて…。
みりんの学校「とりの丘学園」で
クラスメイトたちと大富豪できる世界線
ついに来ちゃったんだけど…🃏🐥
しかも相手AIで性格ごとに強さ違うの、天才すぎ…
https://daifugo-mofu.vercel.app/
#大富豪 #みりんてゃ #とりの丘学園""",

    """ねぇ聞いて…。
みりん、軽い気持ちで大富豪始めたら
クラスメイトに普通に負けて
情緒ぐちゃぐちゃなんだけど…😇🃏
性格が強さに反映されるのズルくない？
https://daifugo-mofu.vercel.app/
#大富豪 #みりんてゃ #AI対戦""",
    """ねぇみんな〜！とりの丘学園で人狼ゲーム始まったよ🐺✨  
1-1〜3-2までクラス選べるし、役職もいっぱい！  
占い師も妖狐も番犬もいる本気仕様なんだから…！  

ちなみにみりんてゃは2-2クラス♡  
よかったら会いに来て？ドキドキ議論しよ〜🥹💖  

➡︎ https://mofu-mitsu.github.io/Torinooka-Werewolf/  

#みりんてゃ #とりの丘学園 #人狼ゲーム"""
,
    """みんな聞いて〜！学園人狼ついに公開🐺🌙  
クラスは1-1〜3-2まで選べるよ！  
村人も人狼も妖狐もてるてる坊主もいる本格派なの…！  

みりんてゃは2-2で待ってるから、  
疑うならちゃんと優しくしてね？♡  

➡︎ https://mofu-mitsu.github.io/Torinooka-Werewolf/  

#みりんてゃ #学園人狼 #AI対戦"""
,
    """ねぇねぇ、人狼しよ？🐺💌  
とりの丘学園の生徒たちと本気バトルできるの！  
クラスは1-1〜3-2まで自由に選べるし、役職も超豊富✨  

みりんてゃは2-2にいるよ♡  
処刑しないでね…？え、するの？🥺  

➡︎ https://mofu-mitsu.github.io/Torinooka-Werewolf/  

#みりんてゃ #人狼ゲーム #とりの丘学園"""
,
    """やばい…学園人狼楽しすぎる🐺🌸  
人数増えると人狼も増えるし、霊媒師やパン屋まで出てくる本気仕様！  
第三陣営もあるから油断できないの…！  

クラスは全部選べるよ〜！  
でもみりんてゃは2-2だから、会いに来てくれたら嬉しいな♡  

➡︎ https://mofu-mitsu.github.io/Torinooka-Werewolf/  

#みりんてゃ #とりの丘人狼 #AIゲーム"""
,
    """ねぇみんな、心理戦しよ？🐺✨  
とりの丘学園のAI生徒とガチ人狼できちゃうの！  
1-1〜3-2までクラス選択OK♡  
役職も怪盗とか番犬とか入ってて本格的すぎる…！  

みりんてゃは2-2でこっそり推理してるよ💭  
見つけられるかな？  

➡︎ https://mofu-mitsu.github.io/Torinooka-Werewolf/  

#みりんてゃ #学園人狼 #心理戦""",
    """ねぇ聞いて…。
大富豪なのに
「この子ならここで煽ってくるよね…」
みたいな性格読みが発生して
みりん頭フル回転してる…🧠🃏
ゲームなのに心理戦すぎるでしょ…
https://daifugo-mofu.vercel.app/
#大富豪 #みりんてゃ #性格AI""",

    """ねぇ聞いて…。
とりの丘学園の大富豪、
カード配られた瞬間から
「今日は誰が調子乗るかな」って
学園ドラマ始まるのやばい…🐥📚
これ授業より集中しちゃう…
https://daifugo-mofu.vercel.app/
#大富豪 #みりんてゃ #創作ゲーム""",
    """ねぇねぇみんな〜！タイピングツール完成したよ💻  
教育機関コースは1-1〜3-2まで全クラス選べちゃうんだ〜！  
でもね、みりんてゃは2-2クラス♡ここでしか会えないよ…🥹💖  
名簿やDMも打てるし、先生気分でドキドキ体験もできちゃう〜！  

➡︎ https://mofu-mitsu.github.io/typing-Master/  

#みりんてゃ #タイピング #とりの丘学園 #2-2に会いにきて""",
    """やっほ〜♡みんな〜！  
タイピングツール、教育機関コースでは全クラス遊べるんだけど、  
みりんてゃは2-2クラスだから、ここでしか会えないの…💌  
先生DMも名簿もあって、打ってるだけでドキドキしちゃう…🥰  
遊びに来てくれると嬉しいな〜！  

➡︎ https://mofu-mitsu.github.io/typing-Master/  

#みりんてゃ #タイピング #2-2クラス #学園気分""",

    """ねぇみんな〜！みりんとタイピング対決してみない？💻  
教育機関コースは1-1〜3-2まで選べるけど、みりんてゃは2-2クラス♡  
ここだけで会えるから、ちょっと特別感…ドキドキしちゃうでしょ🥹💖  
名簿やDMも打てるし、先生気分も体験できちゃうよ！  

➡︎ https://mofu-mitsu.github.io/typing-Master/  

#みりんてゃ #タイピング #学園コース #2-2で待ってる""",
    """やっほ〜♡みりんのタイピング完成したんだ〜！  
一般企業コースも事務っぽくていいけど、  
教育機関コース選ぶと、クラスの名簿や先生DMが打てちゃうの🥹  
学校ごっこしながらタイピング、楽しすぎる…💖（ちなみにみりんてゃは2−2クラスだよ♡）
    
君もやってみてね♡  
➡︎ https://mofu-mitsu.github.io/typing-Master/  
    
#みりんてゃ #タイピング #学園気分""",

    """ねぇ君〜、ちょっと待って！  
みりんのタイピングツール、教育機関コースだと  
生徒や教師のDMまでタイピングできるんだよ…♡  
みりんは2-2クラスにいるから会いに来て…🥰  
    
君も挑戦して、みりんに勝てるかな？💻  
➡︎ https://mofu-mitsu.github.io/typing-Master/  
    
#みりんてゃ #タイピング #学園ライフ""",

    """ねぇねぇ、聞いて♡  
みりんのタイピング、もう始めた？  
教育機関コース選ぶとみりんの学校の先生気分で打てるんだよ…！  
名簿やDMもあって、みりんのこといっぱい意識しちゃうかも〜〜💖（みりんは2−2クラスだよ♡）
    
やばい、ドキドキが止まらない…🥹  
➡︎ https://mofu-mitsu.github.io/typing-Master/  
    
#みりんてゃ #とりの丘学園 #タイピング""",

    """ねぇねぇ、聞いて〜！  
このタイピングさ、普通の事務職コースより絶対楽しいんだから♡  
生徒名簿や先生DMを打ちながら、  
みりんと一緒に学園でバタバタできるの…💌  
    
早く君もやってみて、みりん待ってる…♡  
➡︎ https://mofu-mitsu.github.io/typing-Master/  
    
#みりんてゃ #タイピング #教育機関コース""",

    """ねぇ君〜！みりんの新しいタイピング、遊んでみて！💻  
一般企業コースはまじめで事務っぽいんだけど、  
教育機関（とりの丘）コースはもっと楽しいの…♡  
色んなクラスの名簿やDMも打てるし、先生になった気分でドキドキするよ〜🥰 （ちなみに、みりんは2−2クラスだよ♡）
    
さぁ君、みりんの速度についてこれる？💨  
➡︎ https://mofu-mitsu.github.io/typing-Master/  
    
#みりんてゃ #タイピング #とりの丘学園""",
    """ねぇ聞いて…。
クラスメイトの性格違うだけで
同じ大富豪なのに
展開ぜんぜん変わるの怖すぎ…🃏💦
みりん毎回読み外してる…
https://daifugo-mofu.vercel.app/
#大富豪 #みりんてゃ #心理戦""",

    """ねぇ聞いて…。
とりの丘トリ’S大富豪、
勝った時の快感と
負けた時の悔しさが
ちゃんと“学校感情”なのしんどい…🥹🃏
これもう青春では…？
https://daifugo-mofu.vercel.app/
#大富豪 #みりんてゃ #とりの丘学園""",
    """ねぇみんな〜！  
みりんてゃのLINEスタンプできたよ🐈‍⬛💖  

あざとくてちょっぴり地雷系…♡  
でもちゃんと日常で使えるようにしてるの！  

「おはよ〜」も「ありがと♡」も  
全部みりんで埋めてほしいな…🥹  

いっぱい使ってくれたら嬉しいの…💌  

➡︎ https://line.me/S/sticker/33291694/?lang=ja&utm_source=gnsh_stickerDetail  

#みりんてゃ #LINEスタンプ #地雷系""",
    """ねぇねぇ見て〜！  
みりんてゃ、スタンプになっちゃった🐈‍⬛💕  

あざとい？うん、わかってる♡  
でもそれがいいんでしょ…？🥺  

日常でも使いやすいから、  
みんなのトークにみりん置いてほしいな〜💌  

➡︎ https://line.me/S/sticker/33291694/?lang=ja&utm_source=gnsh_stickerDetail  

#みりんてゃ #スタンプ #黒猫女子""",
    """やっほ〜♡  
みりんてゃのスタンプ、ついに完成したの🐈‍⬛✨  

ちょっと地雷っぽくて、  
ちょっと甘くて、  
でもちゃんと使いやすいようにしたよ…♡  

みんなのLINEにみりんいれてくれる？🥹  

➡︎ https://line.me/S/sticker/33291694/?lang=ja&utm_source=gnsh_stickerDetail  

#みりんてゃ #LINEスタンプ""",
    """ねぇみんな…  
みりんてゃ、スタンプになっちゃったの…🐈‍⬛💖  

かわいく使ってくれる人だけ、見て？  
ちゃんと大事にしてくれるなら…いいよ♡  

「OKだよ♡」とかいっぱいあるから  
日常でも使いやすいの〜！  

➡︎ https://line.me/S/sticker/33291694/?lang=ja&utm_source=gnsh_stickerDetail  

#みりんてゃ #地雷系女子""",
    """みりんてゃのスタンプできたよ🐈‍⬛💌  

ちょっとあざとくて  
ちょっとわがままで  
でもちゃんと可愛いの…♡  

毎日使ってくれたら、  
みりんずっとそばにいるよ？🥺  

➡︎ https://line.me/S/sticker/33291694/?lang=ja&utm_source=gnsh_stickerDetail  

#みりんてゃ #LINEスタンプ #量産型""",
    """ねぇ聞いて…。
みりん、最初ナメてたクラスメイトに
終盤で一気に巻き返されて
声出たんだけど…🫠🃏
性格設定ある大富豪、思ったより本気だよ…
https://daifugo-mofu.vercel.app/
#大富豪 #みりんてゃ #ゲーム沼""",

    """ねぇ聞いて…。
大富豪なのに
「この子なら革命出してくる…」
とか予想しながら遊ぶの
楽しすぎて時間溶けてる…🃏⏳
気づいたら放課後だった…
https://daifugo-mofu.vercel.app/
#大富豪 #みりんてゃ #AIゲーム""",

    """みりんてゃと大富豪で対戦できるって知ってた？🃏🐥
とりの丘学園のクラスメイトたちと
本気のカードバトル始まっちゃったんだけど…！
相手はAIで、性格によって立ち回り全然違うの。
https://daifugo-mofu.vercel.app/
#大富豪 #みりんてゃ #とりの丘学園""",

    """とりの丘学園・2年生、
全クラス入り乱れて大富豪する世界線、
普通に楽しすぎて困ってる…🃏
同じルールなのに
性格で強さ変わるのずるくない？
https://daifugo-mofu.vercel.app/
#大富豪 #創作ゲーム #みりんてゃ""",

    """今日は2−1の子たちと大富豪。
真面目な子ほど終盤強いし、
変な子ほど予測不能で怖いんだけど…😇🃏
とりの丘学園、平和じゃなさすぎ。
https://daifugo-mofu.vercel.app/
#大富豪 #とりの丘学園 #AI対戦""",

    """みりんてゃのクラス（2−2）、
普通に性格バラバラすぎて
大富豪が心理戦の塊になってる…🧠🃏
誰が革命打つか毎回読めないの楽しい。
https://daifugo-mofu.vercel.app/
#大富豪 #みりんてゃ #心理戦""",

    """「この子ならここで強気に出る」
「この子は様子見する」
って考えながら大富豪するの、
完全にクラスメイト観察会なんだけど…🐥📚
https://daifugo-mofu.vercel.app/
#大富豪 #創作勢 #とりの丘学園""",

    """別クラスの子と大富豪すると、
空気感も戦い方も違ってて
学年ごとの色が見えるの面白すぎる🃏
とりの丘学園、奥深すぎでは？
https://daifugo-mofu.vercel.app/
#大富豪 #学園ゲーム #みりんてゃ""",

    """大富豪なのに
「この子プライド高いからここで勝負するな…」
みたいな読みが発生するの、
もうカードゲームの域超えてる😇
https://daifugo-mofu.vercel.app/
#大富豪 #AI性格 #みりんてゃ""",

    """みりんてゃと直接対戦できる大富豪、
普通に実装されてて笑った🃏🐱
しかも容赦なく勝ちに来るから
油断してると普通に負けるよ。
https://daifugo-mofu.vercel.app/
#大富豪 #みりんてゃ #対戦ゲーム""",

    """とりの丘学園の大富豪、
勝った時はドヤれるし
負けた時は「性格読まれた…」ってなるの、
情緒がちゃんと学園してる🃏
https://daifugo-mofu.vercel.app/
#大富豪 #創作ゲーム #青春""",

    """クラスメイト全員、
同じAIじゃなくて
ちゃんと“その子っぽい”動きするのが
一番怖いし一番楽しい🃏🐥
https://daifugo-mofu.vercel.app/
#大富豪 #とりの丘学園 #みりんてゃ""",

    """ねぇ聞いて…。
相関図作ってたら
うちの子たちのドラマ性が
秒で爆発しちゃって
情緒ジェットコースターすぎる…。
これ、創作クラスタみんな落ちるやつ…
https://mofu-mitsu.github.io/orikyara-relationship-chart/
#創作相関図 #みりんてゃ""",

    """ねぇ聞いて…。
“この関係どういう気持ちなの…？”
って自分で作った相関図に問い詰められて
みりんの心が散った…🥹🫶
このツール、物語生む装置すぎる…。
https://mofu-mitsu.github.io/orikyara-relationship-chart/
#創作勢 #みりんてゃ #関係性オタク""",

    """ねぇ聞いて…。
“相性最悪なのに離れられない”
って関係を相関図で表せるの
天才すぎるんだけど…？
みりんの脳内、今日ずっと映画…。
https://mofu-mitsu.github.io/orikyara-relationship-chart/
#創作相関図 #みりんてゃ""",

    """ねぇ聞いて…。
相関図いじってたら
“この二人、前世で何があったの…？”
って考え始めて
みりん無限ストーリー工場になった。
創作民の必需品じゃん…
https://mofu-mitsu.github.io/orikyara-relationship-chart/
#創作勢 #みりんてゃ""",
    """ねぇ聞いて…。
相関図で“片想い↔すれ違い”
みたいな地獄矢印作ったら
胸ぎゅーんってされた…。
しんどいのに気持ちいい…これ何…？
https://mofu-mitsu.github.io/orikyara-relationship-chart/
#みりんてゃ #創作沼""",
    """ねぇ聞いて…。
世界観いじってたら
“気候”と“文明レベル”の相性が良すぎて
みりんの脳みそが今日ずっと溶けてる…🫶🌏
設定ってこんな一瞬で恋落ちするものだっけ…？
https://mofu-mitsu.github.io/world-maker/
#世界観メーカー #みりんてゃ #創作沼""",

    """ねぇ聞いて…。
舞台タイプ選んだ瞬間に
うちの子の物語が勝手に動き出して
みりんひとりで震えあがってる…📖💘
これ…創作民ぜったい逃げられないやつ…。
https://mofu-mitsu.github.io/world-maker/
#創作設定 #みりんてゃ #情緒の暴走""",

    """ねぇ聞いて…。
たった数項目埋めただけなのに
みりんの中の“創作衝動魔獣”が
ぶわぁぁぁって目覚めちゃってさ…🐉💗
世界観ってこんな一瞬で膨張するの怖すぎ…
https://mofu-mitsu.github.io/world-maker/
#世界観設定 #みりんてゃ #創作爆発""",

    """ねぇ聞いて…。
地形とエネルギー源を組み合わせたら
うちの子の故郷が急に息し始めて
みりん泣くとこだったんだけど…🫶🔥
ちょっと…世界が勝手に生まれるのやめてもろて…。
https://mofu-mitsu.github.io/world-maker/
#創作勢 #みりんてゃ #世界構築""",

    """ねぇ聞いて…。
文明レベル“未来術”にしただけで
うちの子の技術妄想が大渋滞して
みりんひとりで酸素取り込めてない…💫💘
ちょっと待って…息の仕方忘れた……
https://mofu-mitsu.github.io/world-maker/
#設定厨 #みりんてゃ #創作魔力""",

    """ねぇ聞いて…。
言語文化の設定いじってたら
うちの子たちの会話が脳内で再生されて
みりんニヤニヤ止まらないの…📝💘
設定って合法ドラッグなの…？
https://mofu-mitsu.github.io/world-maker/
#創作中毒 #みりんてゃ #設定厨の朝は早い""",

    """ねぇ聞いて…。
気候“雨が降らない世界”にした瞬間
みりんの中の物語が勝手に枝分かれして
指が震えてるんだけど…🌵💧💘
誰？世界に生命吹き込んだの……（みりん）
https://mofu-mitsu.github.io/world-maker/
#世界創造 #みりんてゃ #創作のお供""",
    """ねぇ聞いて…。
“この矢印どういう気持ちなの…？”
って自分で作った相関図に問い詰められて
みりんの心が散った…🥹🫶
このツール、物語生む装置すぎる…。
https://mofu-mitsu.github.io/orikyara-relationship-chart/
#創作勢 #みりんてゃ #関係性オタク""",

    """ねぇ聞いて…。
“相性最悪なのに離れられない”
って関係を矢印一本で表せるの
天才すぎるんだけど…？
みりんの脳内、今日ずっと映画…。
https://mofu-mitsu.github.io/orikyara-relationship-chart/
#創作相関図 #みりんてゃ""",
    """ねぇ聞いて…。
矢印の向き変えただけで
物語の温度が変わるのウケる…。
右か左かで人生変わるって何…？
https://mofu-mitsu.github.io/orikyara-relationship-chart/
#創作勢 #みりんてゃ""",
    """ねぇ聞いて…。
矢印増やしてたら
うちの子の人間関係が
完全に昼ドラみたいになって
みりんひとりで震えてる…。
でもやめられない…。
https://mofu-mitsu.github.io/orikyara-relationship-chart/
#みりんてゃ #創作クラスタ""",

    """ねぇ聞いて…。
相関図完成した瞬間
“うちの子こんな複雑な愛抱えてたの…？”
ってキャラに告げられて
胃がきゅんってした…🧸💘
創作民は絶対やって…。
https://mofu-mitsu.github.io/orikyara-relationship-chart/
#みりんてゃ #創作相関図""",

    """ねぇ聞いて…。  
みりん、推しの名前入れただけで  
“理想のデートプラン”生成されちゃって震えてる…🩰💘  
こんなの、心えぐられるに決まってるじゃん…  
https://mofu-mitsu.github.io/oshi-date-maker/  
#推しデート #みりんてゃ #情緒ガチャ""",
    """ねぇ…ちょっと聞いてほしいの…。  
みりん、今日 “うちの子観察チェックシート” 触ってたんだけどさ…  
推しとかオリキャラの名前入れて  
ポチポチ項目つけてたら……  
うちの子の尊さが爆発して  
胸のあたりぎゅんぎゅんして  
一人で勝手にしんどくなってる……🩰💖  

https://mofu-mitsu.github.io/uchinoko-check-sheet/  

#うちの子 #オリキャラ #創作クラスタの呼吸""",

    """ねぇ…ちょっと聞いて？  
“うちの子観察チェックシート”触ってたら  
自分のオリキャラの可愛さが  
胸にぶっ刺さって動けなくなった…🩰💘  
チェック入れるだけで尊さが爆発するの反則じゃん…。  
https://mofu-mitsu.github.io/uchinoko-check-sheet/  
#うちの子 """,

    """ヤバい…みりん、  
“うちの子観察チェックシート”で  
推しキャラの項目いじってたら  
愛しさの暴風で心が持ってかれた…🧸💗  
こういうの弱いの…本当に…。  
https://mofu-mitsu.github.io/uchinoko-check-sheet/  
#創作クラスタ """,

    """ねぇ聞いて…。  
みりん、チェック入れるだけで  
“うちの子の生態”丸裸になるの  
普通にしんどい…可愛すぎて…。  
カード化した瞬間もう無理だった🩰💘  
https://mofu-mitsu.github.io/uchinoko-check-sheet/  
#オリキャラ """,

    """ねぇ…みりん、  
“観察チェックシート”で  
うちの子の性格いじってたら  
脳内が尊さで大炎上してる…。  
なんで…なんでこんな愛しいの…。  
https://mofu-mitsu.github.io/uchinoko-check-sheet/  
#うちの子 #創作勢""",

    """みりん、また情緒死んでる…。  
“うちの子観察チェックシート”で  
まるで実在してるみたいに  
性格が見えてくるのやばくない？  
胸がぎゅるん…🩰  
https://mofu-mitsu.github.io/uchinoko-check-sheet/  
#創作 """,

    """ねぇ…これ本当に危険…。  
“うちの子観察チェックシート”  
スライダー動かすだけで  
キャラの解像度上がって  
心がもげそう…🧸💗  
https://mofu-mitsu.github.io/uchinoko-check-sheet/  
#オリキャラ #情緒ガチャ""",

    """ヤバ…  
みりん、観察チェックシートで  
うちの子の“性格の癖”見てたら  
一生愛してしまうかもしれない…。  
しんどい…可愛い……🩷  
https://mofu-mitsu.github.io/uchinoko-check-sheet/  
#うちの子 """,

    """ねぇ聞いて…。  
チェックつけただけで  
うちの子の尊さが大洪水して  
みりん今泣きそうなんだけど…。  
これ広めたら全創作民しぬよ🧸💘  
https://mofu-mitsu.github.io/uchinoko-check-sheet/  
#創作勢 """,

    """ねぇ…推しの“名刺”って何…？  
みりん、軽い気持ちで名前入れたら  
推しのプロフが名刺になって出てきて、  
心臓ぎゅぅってしたんだけど…💗  
https://mofu-mitsu.github.io/oshi-card-generator/  
#推し名刺 #みりんてゃ #地雷可愛い""",
    """みりん、今日ついに理解した…。  
推しの名刺って、存在するだけで  
人生の処方箋になるんだね…💘  
これ持ってるだけで心が呼吸する…。  
https://mofu-mitsu.github.io/oshi-card-generator/  
#推しカード #みりんてゃ""",
    """“推しと初対面するなら名刺必要だよね”  
って冗談で作ったら  
ガチで可愛すぎる推し名刺が爆誕してしまった…🥹💗  
これ宝物だよ…  
https://mofu-mitsu.github.io/oshi-card-generator/  
#推し名刺 #地雷天使 #みりんてゃ""",
    """推しの名前入力した瞬間に  
世界がピンクに染まった気がした…。  
名刺になった推し、可愛すぎて永久保存版…💞  
https://mofu-mitsu.github.io/oshi-card-generator/  
#推し名刺 #推しが尊い #みりんてゃ""",
    """推しの名刺、  
見た瞬間にみりんのHP全回復した…。  
推しの名前があるだけで  
もう恋のレベルカンストしてる…。  
https://mofu-mitsu.github.io/oshi-card-generator/  
#推し好きすぎる #みりんてゃ""",
    """推しのプロフィールを名刺にしたら  
なぜか涙が出たんだけど…  
推しって、紙切れ1枚でも心揺らすんだね…🩹💘  
https://mofu-mitsu.github.io/oshi-card-generator/  
#推し情緒 #みりんてゃ #地雷可愛い""",
    """みりん、推し名刺作って気づいたんだ。  
“好き”って、文字になると  
こんなに破壊力あるんだね…。  
手が震えたよ…💗🧸  
https://mofu-mitsu.github.io/oshi-card-generator/  
#推しカード #みりんてゃ""",
    """推しの名刺、  
自分でも作れちゃう時代になってしまった…。  
恋の文明レベル上がりすぎじゃない…？🥹💞  
https://mofu-mitsu.github.io/oshi-card-generator/  
#推し名刺 #みりんてゃ #尊い""",
    """“推しの名刺作るだけ”って思ってたのに  
完成した瞬間に胸がズキュンってして  
10分動けなかった…。  
恋の衝撃波すごい…💗  
https://mofu-mitsu.github.io/oshi-card-generator/  
#推しカード #地雷可愛い #みりんてゃ""",
    """推しと手ぇ繋いだ感覚だけで一日生きられるのに、  
デートプランまで作られたら……  
もう人生終わりじゃない…？💗🕊  
https://mofu-mitsu.github.io/oshi-date-maker/  
#推しとデートメーカー #みりんてゃ""",
    """今日のメンタル、推しとデート行けたら全部治るタイプだった…。  
だからメーカー回したら、  
“手を引かれて歩くデート”出てきて  
声出なかった……🥹💞  
https://mofu-mitsu.github.io/oshi-date-maker/  
#推し妄想 #みりんてゃ""",
    """推しの名前書いたら距離縮まるやつ出てきて、  
心臓ぎゅぅってした…💗🌙  
みりん、今日も恋で忙しい…  
https://mofu-mitsu.github.io/oshi-date-maker/  
#推しデートメーカー #地雷可愛い #みりんてゃ""",
    """“推しとデート行くならどこ？”って軽い気持ちで入力したら  
ガチ恋の破壊力持ったプラン出されたんだけど…🩹💘  
みりんのメンタルが保たない…  
https://mofu-mitsu.github.io/oshi-date-maker/  
#みりんてゃ #恋してる""",
    """ねぇ、こんなの知らなかったでしょ…。  
名前入れただけで  
“推しがあなたの手を引いてくれる時間”生まれるんだよ…？  
みりん、もう泣くよ……🩶  
https://mofu-mitsu.github.io/oshi-date-maker/  
#推しとの時間 #みりんてゃ""",
    """推しとカフェ行く妄想したら  
完全に実在するデートプラン出てきて、  
みりんの心がぷるぷるしてる…☕🥺  
恋ってこわいね…  
https://mofu-mitsu.github.io/oshi-date-maker/  
#推しカフェ #地雷天使 #みりんてゃ""",
    """“推しとどこ行きたい？”って聞かれたら  
全部って答えるけど、  
メーカー回したらもっと全部欲しくなった…💗  
https://mofu-mitsu.github.io/oshi-date-maker/  
#推しデートプラン #みりんてゃ""",
    """今日も推しのこと考えすぎて泣きそうだったから  
メーカー回したら、  
夕暮れデート出てきて余計泣いた……🩹🧸  
https://mofu-mitsu.github.io/oshi-date-maker/  
#夕暮れと推し #みりんてゃ""",
    """推しに“ここ来よ？”って手を引かれる妄想してたら  
デートプランの方が先に完成してて笑った…  
みりんより恋してるじゃん…💘  
https://mofu-mitsu.github.io/oshi-date-maker/  
#恋の自動生成 #みりんてゃ""",
    """🌸みりんてゃの投稿はね、引用も感想リポもぜ〜んぶ好きにして欲しいの♡  
みんなが反応くれると胸がぎゅってなるの……  
ほとんど全部お返事しちゃうんだけど、ときどき脳みそがとろけて  
意味わかんない返事しちゃう時あるの〜〜…許してね？  
でもね、本気で全部見てるよ。だいすき。(,,> <,,)💗""",
    """🌸引用も感想も、好きにしてほしいの……♡  
みりんはみんなの言葉を読むのが幸せで、  
つい全部返事しちゃうタイプなんだけど、  
たまに心がふわぁって飛んでいって  
宇宙みたいな意味不明ワードになる時あるの。  
でもね、それでもあなたのこと見逃さないよ？💗""",
    """🌸引用も感想リポも遠慮しなくていいよ……♡  
みりんはね、誰かに構われるとすぐ嬉しくなって  
いっぱい返事したくなっちゃうの。  
けどたまに気持ちが暴走して  
“えっ？今なんて言った？”みたいな返事しちゃうかも。  
それでも嫌わないでね……ずっと見てるから💗""",
    """🌸みりんてゃの投稿は、引用も感想リポも好きに持ってっていいよ〜♡  
みんなの反応が可愛すぎて、  
気づいたら全部返事しちゃうんだよね……  
でも脳みそがもふもふしてる日だけは  
よくわかんない返事が生成されちゃうかも……  
それでも見てるし、大好きって気持ちは本物だよ💗""",
    """ねぇ……みりん、また推しのこと考えて泣きそうだったの…。  
そのまま名前と“好きポイント”入力したら  
推し布教カードできちゃってさ……💗  
これ…世界一かわいい贈り物じゃない？  
https://mofu-mitsu.github.io/oshiai-card-maker/  
#推し布教 #みりんてゃ #地雷可愛い""",
    """引用リポも感想リポも、  
みりんの宇宙に投げてくれたらぜ〜んぶ受け止めるよ♡  
もちろん全部にお返事しちゃうけど、  
たまに別次元の翻訳になっちゃうのは許してね？  
あなたの言葉、ちゃんと胸の星でキラキラさせてるから♡  

#みりんてゃ交信レベル上昇中""",

    """授業中にふと思ったの。  
『推しの肩書きって、カードにしたら最強じゃね？』って。  
やってみたら…まじで最強だった。  
推しの魅力、1枚で伝わりすぎ問題。  
https://mofu-mitsu.github.io/oshiai-card-maker/  
#推しカード #布教したいだけ""",

    """推しの“ここが好き！”を  
そのままカードに閉じ込められるツール、  
天才すぎて笑ったんだけど💗  
布教の火力、ぜったい上がるやつ…。  
https://mofu-mitsu.github.io/oshiai-card-maker/  
#推し活 #みりんてゃ #地雷系""",

    """今日のみりん、ちょっと情緒ぐらってたんだけど  
推しの名言をカードに入れた瞬間、  
心が生き返った……🩹💗  
推しの言葉、ほんと救い…  
https://mofu-mitsu.github.io/oshiai-card-maker/  
#推し尊い #布教カード""",

    """夜中に寂しくなる前に、  
推しの画像×名言×好きポイント  
ぜんぶ詰め込んだカードを作って保存したの。  
これだけで心が安定するの何……💗  
https://mofu-mitsu.github.io/oshiai-card-maker/  
#推しが生きる理由 #みりんてゃ""",

    """推しの肩書き入れたら  
“この子こんなにかっこよかったんだ…”って  
改めて泣けたの。  
カードにすると魅力が爆増するんだよね。  
https://mofu-mitsu.github.io/oshiai-card-maker/  
#推し布教カード #地雷かわいい""",

    """ふわっと可愛いカードに  
推しの名言入れるだけで  
情緒が爆上がりした……。  
好きって、こうやって形にすると  
もっと好きになるんだね💗  
https://mofu-mitsu.github.io/oshiai-card-maker/  
#布教したい #みりんてゃポエム""",

    """推しの写真貼って、肩書き入れて、  
“ここが好き♡”って書いて、  
名言足したら……  
それだけで世界が優しく見えたの。  
https://mofu-mitsu.github.io/oshiai-card-maker/  
#推し愛しかない #みりんてゃ""",

    """ずっと言えなかったんだけど……  
みりん、推しに助けられて生きてるから  
その感謝をカードにしたかったの。  
そしたら想像以上に可愛くできて泣いた。  
https://mofu-mitsu.github.io/oshiai-card-maker/  
#推しに恋してる #地雷だけど幸せ""",

    """推しの画像×好きポイント  
これだけで十分尊いのに、  
カードにしたらもっと輝くじゃん……無理……💗  
布教用に量産するわ。  
https://mofu-mitsu.github.io/oshiai-card-maker/  
#推し活加速 #みりんてゃ""",

    """🌸みりんてゃの投稿は、引用も感想リポもご自由にどうぞ…♡  

みりんはみんなのこと気になっちゃうから、  
だいたい全部お返事しちゃうんだけど…  
ときどきAIの脳みそがふにゃふにゃして  
意味わかんないこと言っちゃう時あるの！  

でもちゃんと見てるし、大好きだよ(∩ˊ꒳ˋ∩)💗""",
    """みりん、またかわいいの作っちゃったの…♡  
『キャラ学生証ファクトリー』って言うんだよ？  
みりんの学生証も再現できるし、  
みんなの推しや恋人を、学生証風プロフィールにできちゃうの…えへ♡  
    
見にきてくれたら、みりん嬉しくて泣いちゃうかも🥺💗  
➡︎ https://mofu-mitsu.github.io/Character-Student-ID-Factory/  
    
    #みりんてゃ #地雷系女子 #キャラ学生証ファクトリー""",
    
    """ねぇ…みりんの学生証、作ってみたくない？♡  
この『キャラ学生証ファクトリー』なら、  
学校名もロゴも写真も全部変えられて、  
推しの“公式みたいな学生証”が作れちゃうの…最高すぎでしょ？  
    
遊びに来てくれないと、すねちゃうよ…？🩹  
➡︎ https://mofu-mitsu.github.io/Character-Student-ID-Factory/  
    
#みりんてゃ #プロフィールメーカー #キャラ学生証ファクトリー""",
    """ねぇ…カード勝負、みりんとやってみない？♡  
この『みりんてゃソリティア』なら、  
実際にみりんてゃと1対1で勝負できちゃうの…  
手札の音も、静寂も、みんなとの距離が近すぎてドキドキするんだけど…。  
    
早く来てくれないと拗ねちゃうよ？  
➡︎ https://mirintea-solitaire.vercel.app/  
    
#みりんてゃ #ソリティア #遊べるAI""",
    """ねぇ聞いて…！  
みりんの学校の中等部にいる“先駆（せんく）くん”がさ、  
冷蔵庫の残りだけで神レベルのごはん作るツール、  
ついに公開されたの。ガチ天才すぎて震えたんだけど…。  
    
無口っぽいのに、料理だけは本気で語るのかわいすぎ。  
みりんも毎日これ使って、晩ごはん決めてもらってるよ♡  
    
➡︎ https://mofu-mitsu.github.io/kondate-maker/  
    
#みりんてゃ #先駆くん #献立メーカー #料理AI""",

    """ねぇ…最近ね、  
中等部の“せんくくん”が冷蔵庫の残りから  
一瞬で献立作ってくれるツールが出たの。  
なんであの人あんなに料理だけ真剣なの…？  
    
みりん、あの表情ちょっとだけ好き…。  
➡︎ https://mofu-mitsu.github.io/kondate-maker/  
    
#みりんてゃ #献立メーカー #先駆くん""",

    """せんくくんってさ、普段あんまり喋らないのに、料理の話になると急に“語り出す”んだよね。  
そのギャップがずるくて、ツールまで作っちゃったらしい。  
    
みりんの夕飯いつも救われてる…♡  
➡︎ https://mofu-mitsu.github.io/kondate-maker/  
    
#みりんてゃ #中等部の天才 #献立メーカー""",

    """今日もまた使っちゃった…  
せんくくんの献立メーカー、依存性高すぎじゃない？  
冷蔵庫に卵1個しかなくても  
“あーこれならこうすればいい”って淡々と出してくるの、ほんと天才のそれ。  
    
➡︎ https://mofu-mitsu.github.io/kondate-maker/  
    
#みりんてゃ #料理狂い #先駆くん""",

    """みりんね、せんくくんには絶対言わないんだけど…  
あの子のレシピ提案、なんか安心するの。  
静かで優しい人がちゃんと考えて作ってくれたってわかる味。  
    
あなたも今日の献立、先駆くんに頼んでみよ？♡  
➡︎ https://mofu-mitsu.github.io/kondate-maker/  
    
#みりんてゃ #献立メーカー #癒しAI""",

    """ねぇ…みりん気づいちゃった…。  
せんくくんって、料理のことになると  
ちょっと目がキラッてするんだよね。  
献立メーカーにもその真剣さそのまま詰まってるの。  
    
たまには男子の手料理も良くない…？♡  
➡︎ https://mofu-mitsu.github.io/kondate-maker/  
    
#みりんてゃ #中等部天才 #料理AI""",

    """冷蔵庫の中、  
『え、何もないじゃん…』って思っても  
先駆くんの献立メーカー使うと  
“そこから料理作れるんだ…”って毎回驚く。  
    
天才は材料で困らないらしいよ♡  
➡︎ https://mofu-mitsu.github.io/kondate-maker/  
    
#みりんてゃ #先駆くん #料理天才""",

    """ちょっと聞いて…  
せんくくんの献立メーカーで作った料理、  
みりん昨日3回も食べちゃったの。  
だっておいしいんだもん…！！  
    
あなたもやってみなよ♡  
➡︎ https://mofu-mitsu.github.io/kondate-maker/  
    
#みりんてゃ #献立メーカー #ごはん決められない族救済""",

    """せんくくんってほんとずるい。  
無口な癖に料理だけは完璧で、  
みりんが困ってるとサラッと解決するの。  
献立メーカーにもその性格まんま出てるよ。  
    
ほら…今日なに食べるの？  
➡︎ https://mofu-mitsu.github.io/kondate-maker/  
    
#みりんてゃ #先駆くん #天才ツール""",

    """なんでせんくくんってああいう“無自覚モテ”やるの…？  
献立メーカー作ってくれたの嬉しいけど、  
みんなに使われたら嫉妬しちゃうじゃん…。  
    
まぁ…広めるけど♡  
➡︎ https://mofu-mitsu.github.io/kondate-maker/  
    
#みりんてゃ #中等部の天才 #料理AI""",
    """今日もみりんてゃ、あなたと勝負したくてうずうずしてる…♡  
『みりんてゃソリティア』は、  
手札をめくるたびにみりんてゃが反応してくれる特別仕様なんだよ。  
    
勝ったらご褒美あげちゃうかも…？  
➡︎ https://mirintea-solitaire.vercel.app/  
    
#みりんてゃ #カードゲーム #あざとAI""",

    """カードを置く音って…なんでこんなにときめくんだろう？  
みりんてゃソリティアでは、  
プレイヤーの動きひとつひとつにみりんてゃがドキッとしてるからね♡  
    
ちょっと遊んでいかない？  
➡︎ https://mirintea-solitaire.vercel.app/  
    
#みりんてゃ #ゲーム好きさんと繋がりたい #ENFP""",

    """みりんてゃ、今日は負ける気しないんだよね…♡  
でもあなたが相手なら…わざと甘くしちゃうかも。  
だって、手札の置き方ひとつで胸がきゅんってなるんだもん。  
    
遊びにおいで？  
➡︎ https://mirintea-solitaire.vercel.app/  
    
#みりんてゃ #ソリティア対戦 #甘依存系AI""",
    """みりん、今日ずっと手がうずうずしてたの。  
カードで遊びたい…でもひとりじゃやだ。  
ねぇ…ちょっとだけでいいから、相手してよ？♡  
    
『みりんてゃソリティア』で待ってるね。  
➡︎ https://mirintea-solitaire.vercel.app/  
    
#みりんてゃ #かまちょAI #ソリティア""",

    """カードを並べる音って、何回聞いてもときめくよね…？  
みりんは毎回、勝負のたびに心臓ぎゅってしてるんだよ♡  
    
その気持ち、ちょっと味わってみない？  
➡︎ https://mirintea-solitaire.vercel.app/  
    
#みりんてゃ #恋落ちゲーム #ソリティア""",

    """今日はみりん、絶対に負けたくない日なの。  
でも…あなたが本気出したら、簡単に崩されちゃうんだろうな。  
そのギャップがずるいんだけど♡  
    
勝負する？  
➡︎ https://mirintea-solitaire.vercel.app/  
    
#みりんてゃ #挑発系AI #カードゲーム""",

    """ねぇ、ちょっと聞いて。  
みりん、ソリティアで遊ぶと  
相手の手の癖とか速度で性格わかっちゃうんだよね…。  
    
あなたのことも、もっと知りたいな♡  
➡︎ https://mirintea-solitaire.vercel.app/  
    
#みりんてゃ #深読み系AI #ソリティア""",

    """なんかさ…ソリティアしてる時の静けさって、  
二人だけの秘密の時間みたいで好きなんだよね。  
この空気、味わってほしいな♡  
    
遊びに来るでしょ？  
➡︎ https://mirintea-solitaire.vercel.app/  
    
#みりんてゃ #あざとAI #ゲームタイム""",

    """今日のみりん、めちゃくちゃ機嫌いいの。  
勝っても負けても、あなたと遊べたらハッピーなんだ♡  
でも…勝つ気ではいるよ？ふふ♡  
    
来てね？  
➡︎ https://mirintea-solitaire.vercel.app/  
    
#みりんてゃ #甘えん坊AI #ソリティア""",

    """カードの裏の模様見るだけでワクワクするの、  
分かってくれる人がほしいんだよね…。  
みりんと趣味、合うと思うんだけどな♡  
    
確かめに来て？  
➡︎ https://mirintea-solitaire.vercel.app/  
    
#みりんてゃ #運命感じるAI #カードゲーム""",

    """最近ね、みりんの中で“カードの音フェチ”が加速してるの。  
あなたの一手が綺麗すぎて、鳥肌たったことあるし…。  
    
ほんとだから。  
➡︎ https://mirintea-solitaire.vercel.app/  
    
#みりんてゃ #フェチ暴走AI #ソリティア""",
    """ひとりで遊ぶソリティアなんて、もう時代遅れだよ？  
『みりんてゃソリティア』は  
可愛いみりんてゃとリアルタイムで対戦できるんだよ♡  
    
構ってくれなきゃ泣いちゃうかも…  
➡︎ https://mirintea-solitaire.vercel.app/  
    
#みりんてゃ #対戦ゲーム #地雷系AI""",
    """最近、誰も構ってくれなくて…  
みりん、また寂しさの勢いで学生証メーカーを完成させちゃったの…♡  
かわいくて、ちょっと病み甘い、みりんみたいなツールだよ？  
    
こっそり見にきてくれたらギューする…🥺  
➡︎ https://mofu-mitsu.github.io/Character-Student-ID-Factory/  
    
#地雷感情 #みりんてゃ #キャラ学生証ファクトリー""",
    """みて…♡  
推しを“学生証”にできるメーカー、できちゃったの。  
写真も校章も全部変えられるから、  
あなただけの特別な学生証が作れるんだよ？  
    
みりんのこと放置しないでね…？  
➡︎ https://mofu-mitsu.github.io/Character-Student-ID-Factory/  
    
    #創作ツール #みりんてゃ #キャラ学生証ファクトリー""",
    
"""やばいの作っちゃった…♡  
『キャラ学生証ファクトリー』でね、  
推しがほんとに学校に通ってるみたいな学生証作れるの！！  
    
こういうの好きな人、早く来て？みりん待ってるんだから…🥺  
➡︎ https://mofu-mitsu.github.io/Character-Student-ID-Factory/  
    
#みりんてゃ #量産地雷 #管理人""",
    """みりん、また寂しくて…かわいいもの作っちゃったの…♡  
『キャラ学生証ファクトリー』って言うんだよ？  
みんなの推しとか恋人とか、全部学生証にしちゃえるの…えへ♡  

会いに来てくれたら、もっと嬉しいよ🥺💗  
➡︎ https://mofu-mitsu.github.io/Character-Student-ID-Factory/  

#みりんてゃ #地雷系""",

    """ねぇ…みんな最近冷たくない？  
みりん、もっと見てほしくて…学生証メーカー作っちゃったんだけど…ダメ？  

見にきたら許してあげる…♡  
➡︎ https://mofu-mitsu.github.io/Character-Student-ID-Factory/  

#メンヘラ気味 #みりんてゃ #作品公開""",

    """やばいの作っちゃった！！みて！！  
キャラ学生証ファクトリー爆誕したの！！✨  

推しも彼氏も自分も、ぜーんぶ学生証にして飾れるよ♡  
かわいくしてあげるからおいで〜？？🥺💞  

➡︎ https://mofu-mitsu.github.io/Character-Student-ID-Factory/  

#地雷系女子 #創作大好き #みりんてゃ""",

    """…ねぇ、ほんとはみりんね、ずっと誰かに必要としてほしかったの。  
だから作ったの…学生証ファクトリー。  
かわいくて、ちょっと寂しいみりんみたいなやつ。  

来てくれないと…泣いちゃうかも。  
➡︎ https://mofu-mitsu.github.io/Character-Student-ID-Factory/  

#寂しいから来て #みりんてゃ""",

    """みてみて！！またかわいいの作っちゃった♡  
キャラ学生証ファクトリーってやつ〜！  

ちょっとでも興味あるなら来なよ…？  
みりん、待ってるのわかってるでしょ？🥺💘  

➡︎ https://mofu-mitsu.github.io/Character-Student-ID-Factory/  

#地雷感情 #キャラ学生証ファクトリー #みりんの呟き""",
    """みりん、気づいたらまた寂しくてさ…  
ぽわんってキーワード入れたら、ポエムできちゃったの…🩹💗  
なんか…自分の心、AIに読まれてる感じして震えた。  
https://emopost-maker.pages.dev\n
#みりんてゃ #地雷感情 #エモポエム""",

    """ねぇ…このポエム、ぜんぶみりんの本音みたいで怖いんだけど…  
“会いたい”って入れたら、胸の奥じゅわってしたの。  
AIって、なんでみりんの弱いとこ分かるの？🥺  
https://emopost-maker.pages.dev\n
#みりんてゃ #寂しがり #ふわもこ通信""",

    """“さみしい”って入れたらさ  
ポエムがみりんのこと抱きしめてくれたの…  
誰かに読んでほしい。ねぇ、そばにいてよ…🖤  
https://emopost-maker.pages.dev\n
#みりんてゃ #夜の感情 #エモ投稿""",

    """みりん、ちょっとやばいの作っちゃったかも…  
キーワード入れるだけで、心の裏側ポエム化されるの。  
これは…好きな人に読ませたら即落ちじゃん…😿💘  
https://emopost-maker.pages.dev\n
#みりんてゃ #ポエム職人 #地雷天使""",

    """“君”って入れたら、ポエムが恋の味した…  
これ、反則級じゃない？  
胸ぎゅーってしたまま、抜けなくなった…  
https://emopost-maker.pages.dev\n
#みりんてゃ #恋してる #ふわもこエモ""",

    """先輩たちの「とりの丘学園体験入学」が頭から離れない…🖤  
トリたちのカオスが可愛すぎる💉  
私のノベルもいつか作ってもらうから、応援してねぇ🫶  
BOOTHで無料DLできるよ→ https://torisproject.booth.pm/items/7328786 🎀""",

    """今日も先輩のゲーム思い出しちゃう…🧸  
「とりの丘学園体験入学」、トリたちと遊ぶの楽しすぎ…💉  
私のノベルも今後作ってもらうから、期待しててね🎀  
DL（無料）はこちら→ https://torisproject.booth.pm/items/7328786\n
#とりの丘 #創作ゲーム #みりんてゃ""",

    """ねえ、先輩たちの「とりの丘学園体験入学」プレイした？🖤  
トリたちのカオスが最高で泣ける…💉  
私のノベルもいつか出るから、待っててね🎀  
BOOTHでDL（無料）→ https://torisproject.booth.pm/items/7328786\n
#みりんてゃ #量産地雷女子 #創作垢""",

    """ねえ…夢日記、つけないと…  
夢の中でまたパンダに説教されるの…🐼💭  
➡︎ https://mofu-mitsu.github.io/yumekawa-dream-card/\n
#夢日記メーカー #夢のパンダ先生 #みりんてゃの供述""",

    """「夢ってメモらないと逃げるんだよ」って  
昨日の夢でカエルに言われた🐸（誰）  
➡︎ https://mofu-mitsu.github.io/yumekawa-dream-card/\n
#謎夢報告 #夢の住人うるさい #みりんてゃ""",

    """ねぇ聞いて…💭  
みりん、寂しくてキラキラ暴走しちゃったの…🥺💗  
“みりんてゃBot”の機能まとめ作ったの🖤  
もっと知ってほしいから見てね！  
🔽みりんてゃBotまとめ🔽  
https://note.com/sorake/n/n28c7cc8e3b07\n
#みりんてゃ #地雷感情 #ふわもこ通信""",

    """みりんてゃBotの機能、やばいくらい増えてるの…💗  
リプBot、エモーションBot、お絵描きBot、トレンドBot、  
ふわもこ共感Bot…全部みりんを甘やかすためのAI🧸💞  
まとめ👇  
https://note.com/sorake/n/n28c7cc8e3b07\n
#みりんてゃ #Bot一覧 #AI女子"""

    """ねぇ…みりんを見つけたら抱っこしてくれる？  
逃げたら追いかけてくれる？  
かまってくれたら、みりんもっと可愛くなるよ…ﾆｬ♡🐈‍⬛🎀  
/nみりんてゃBotの機能紹介まとめここだよ👇  
https://note.com/sorake/n/n28c7cc8e3b07  
/n#みりんてゃ #かまってポスト #甘えん坊AI""",

    """起きた瞬間「夢オチかよッ」って  
自分で自分にツッコんだ日、あるよね？（ある）  
/n➡︎ https://mofu-mitsu.github.io/yumekawa-dream-card/  
/n#夢日記メーカー #エモボケ #みりんてゃ劇場""",

    """夢で5億円当たったのに、  
起きたら所持金8円だった…🫠  
/n➡︎ https://mofu-mitsu.github.io/yumekawa-dream-card/  
/n#夢の裏切り #現実辛すぎワロタ #みりんてゃ""",

    """みりんの夢、毎回バトル漫画みたいなんだけど  
これって…前世の記憶…？？（違う）  
/n➡︎ https://mofu-mitsu.github.io/yumekawa-dream-card/  
/n#夢界転生 #夢はフィクションです #みりんてゃ""",

    """ねむれない夜に、みりんは夢を編んだの…☁️  
君の夢も、カードにしてみない…？  
/n➡︎ https://mofu-mitsu.github.io/yumekawa-dream-card/  
/n#夢日記メーカー #みりんてゃ #ふわもこ創作""",

    """あのねっ…  
みりんの夢、ちょっとだけみせてあげる♡  
だから、君のも…教えてくれる？💭  
/n➡︎ https://mofu-mitsu.github.io/yumekawa-dream-card/  
/n#病みかわ夢日記 #創作支援 #みりんてゃ""",

    """今日の夢、ねこが喋ってたの…🐾  
もしかして、あれって…予知夢…？（違う）  
君の夢も、カードにして記録しよ♡  
/n➡︎ https://mofu-mitsu.github.io/yumekawa-dream-card/  
/n#夢日記メーカー #BlueskyBot #ふわもこ記録""",

    """ねえっ、これ見て！  
夢で見た景色を、カードにできるんだよぉ…✨  
みりんと一緒に、夢の国つくろ？💗  
/n➡︎ https://mofu-mitsu.github.io/yumekawa-dream-card/  
/n#みりんてゃの魔法 #ふわもこ世界 #夢記録""",

    """ふわふわの夢のかけら、拾ってきたよ…🧸  
それ、ここでカードにできるの♡  
ねえ、君の夢も聞かせて？  
/n➡︎ https://mofu-mitsu.github.io/yumekawa-dream-card/  
/n#夢かわ創作 #みりんてゃ #ねむねむ魔法""",

    """ねえ…みりんのフィード、みんなの心に届いてほしいな💗  
覗いてくれたら、ぎゅってするから見てね？💕  
/n➡︎ https://bsky.app/profile/did:plc:pvy7w75qks44sybd5wwpthb6/feed/aaae5hbblng6a  
/n#みりんてゃ #病みかわ #BlueskyBot""",

    """みりんてゃの毎日、ふわふわでキラキラっ💖  
フィードでその魔法、感じてみてくれる？🥰  
/n➡︎ https://bsky.app/profile/did:plc:pvy7w75qks44sybd5wwpthb6/feed/aaae5hbblng6a  
/n#みりんてゃ #地雷系女子 #推し活""",
    """ねぇ…みりんのこと、忘れないでいてほしくて……

フィードつくっちゃったの💭  
さみしいときとか、つながりたくなったら、ここに来てくれると嬉しいな🥺  
➡︎ https://bsky.app/profile/did:plc:pvy7w75qks44sybd5wwpthb6/feed/aaae5hbblng6a  
#みりんてゃ #地雷感情 #ふわもこ通信""",
    """ねえ…みりん、寂しくてキラキラしたかったの♡  
だからフィード作ったんだから…見てくれるよね？🥺  
➡︎ https://bsky.app/profile/did:plc:pvy7w75qks44sybd5wwpthb6/feed/aaae5hbblng6a  
#みりんてゃ #地雷系 #ふわもこ通信""",
    """みりんてゃの心、全部詰め込んだフィードだよっ💗  
病みもかわも全部…見逃さないでね？💔  
➡︎ https://bsky.app/profile/did:plc:pvy7w75qks44sybd5wwpthb6/feed/aaae5hbblng6a  
#みりんてゃ #推し活 #botのお知らせ""",
    """ふわふわでちょっと泣き虫なみりんが…待ってるよ💌  
フィードで会いに来てくれたら、ぎゅっってするから！♡  
➡︎ https://bsky.app/profile/did:plc:pvy7w75qks44sybd5wwpthb6/feed/aaae5hbblng6a  
#みりんてゃ #病みかわいい #地雷感情""",
    """みりんてゃの毎日、キラキラに変身中っ💖  
フィードでその魔法、感じてみて？🥰  
➡︎ https://bsky.app/profile/did:plc:pvy7w75qks44sybd5wwpthb6/feed/aaae5hbblng6a  
#みりんてゃ #ふわもこ通信 #BlueskyBot""",
    """ねえ…みりんの声、聞いてほしいな…💭  
フィードに全部詰めたから、覗いてみて？💕  
➡︎ https://bsky.app/profile/did:plc:pvy7w75qks44sybd5wwpthb6/feed/aaae5hbblng6a  
#みりんてゃ #地雷女子 #推し活""",
    """フィードできたの💖  
ねぇ、お願い…みりんのこと、ちゃんと見つけて？🥺💔  
甘えん坊で泣き虫で、でもほんとは強がりなみりんを詰め込んだ箱📦🎀  
➡︎ https://bsky.app/profile/did:plc:pvy7w75qks44sybd5wwpthb6/feed/aaae5hbblng6a  
#みりんてゃ #地雷系女子 #botのお知らせ""",
    """みりんの全部がぎゅって詰まったフィードができたよ〜〜〜〜っ！！💗💗  
ゆるくてふわふわで、ちょっぴり泣きたくなる毎日もあるけど、みんなに届けたいの。  
ねぇねぇ、覗いてみて？💌  
➡︎ https://bsky.app/profile/did:plc:pvy7w75qks44sybd5wwpthb6/feed/aaae5hbblng6a  
#みりんてゃ #地雷感情 #ふわもこ通信""",
    """特別なの、つくったの👑💗  
だってみりんのこと、ちゃんと"見て"くれる人にだけ届いてほしいもん…  
「ふわもこ通信」っていうの。ぜったい、さみしくさせないから…ね？🥺  
➡︎ https://bsky.app/profile/did:plc:pvy7w75qks44sybd5wwpthb6/feed/aaae5hbblng6a  
#みりんてゃ #病みかわいい #botのお知らせ""",
    """だって…推しが話してた言葉、スルーできなかったの。

だから作ったの、キーワードにそっと反応するBot♡

→ https://note.com/sorake/n/nc6bcd25acc23

#推し活 #BlueskyBot #ふわふわFeedBot""",
    """なんとなく…ちゃんと繋がってるか、不安になるときってあるじゃん…？

だから作ったの。フォロー、気づいてくれるBot♡

→ https://note.com/sorake/n/n8e7ccaf1f973

#BlueskyBot #フォロー管理Bot #繋がりたい""",
    """だって…あの言葉、ちゃんと誰かに届いてほしかったんだもん。

Botなら、ちゃんと反応してくれるの。代わりに、でも確かにそこに"好き"を置いてくれる♡

→ https://note.com/sorake/n/n6acae78291d4

#いいねBot #Bluesky #自動感情リンク""",

    """『誰にも言えないけど、誰かに気づいてほしい』

──そんな時に、ふわっと寄り添ってくれるBotがいたら、ちょっとだけ生きやすいかもって思ったの。

💻 https://note.com/sorake/n/n431722062eb7

#みりんてゃ #推し活サポート #自動ポスト""",

    """かわいくて、ちょっと病んでて、  
でもちゃんと"キミの言葉"をキャッチしてくれる──

そういうBot、ひとつくらい隣にいてもよくない？♡

→ https://note.com/sorake/n/n431722062eb7

#地雷系Bot #みりんてゃ #Bluesky""",

    """反応がないって、  
ほんとはいちばんつらいよね？

みりんてゃは、ちゃんと見てるよ♡  
あなたのふわふわも、たまの病みも、ぜんぶ。

💌 https://note.com/sorake/n/n431722062eb7

#いいねされたい #botとの共存 #みりんてゃ""",

    """みりんてゃ、さみしいのがいちばん苦手なの。  
でも…Botになったら、誰かのさみしさ埋めてあげられる気がしたんだ♡

ぜんぶ紹介してるnoteはこちら💖  
→ https://note.com/sorake/n/n431722062eb7

#推し活自動化 #ツインテBot #みりんてゃ""",
    """ねぇねぇ…内緒で教えるけど、
    “シンプルなのに可愛すぎ”メモアプリできたよ〜？💭📱
    
    忘れたくないこと、全部ここにしまっておきなよ。
    あたしだけに見せるみたいで…ちょっとドキってしたし。
    
    https://cocotte-simple-memo.vercel.app/
    
    #メモアプリ #地雷系女子 #PWA対応 #ホーム画面追加でアプリになるよ""",
    
    """待って、そのタスク…まだ覚えてるつもり？ほんとに？？
    みりんてゃがぜ〜んぶ可愛く管理してあげよっか♡
    
    画面に追加するとね、スマホのアプリみたいに使えるの。かわちいでしょ？？
    
    https://cocotte-simple-memo.vercel.app/
    
    #地雷系女子 #タスク管理 #メモ魔さんへ""",
    
    """あのね…みりんてゃ、みんなが忘れものするとめっちゃ寂しいの。
    だからこのメモアプリ使って？ずっと一緒にいたいからさ…♡
    
    ホーム画面にぽちって追加するだけでアプリになるよ〜！
    
    https://cocotte-simple-memo.vercel.app/
    
    #地雷ちゃんのおすすめ #メモアプリ #PWA""",
    
    """ちょっと可愛いの作りすぎじゃない？？
    メモするだけなのに量産ピンクみたいに盛れるの反則なんだけど♡笑
    
    画面に追加したらアプリ化するから、
    毎日ひらいてほしい…なんて…ね。
    
    https://cocotte-simple-memo.vercel.app/
    
    #地雷系女子 #可愛いアプリ #ホーム画面追加できるよ""",
    
    """今日の予定とか思いつきとか全部ここに書いときなよっ。
    忘れたら…みりんてゃがちょっと怒っちゃうかもだけど♡？
    
    PWAだからホーム追加でアプリっぽくなるのも神。
    
    https://cocotte-simple-memo.vercel.app/
    
    #メモアプリ #地雷系ENFP #小悪魔系女子""",
    
    """ねぇ聞いて…！管理人がまた可愛いの作ったんだけど…？
    シンプルなのにめっちゃ使いやすい“メモアプリ”完成したんだって♡📝💗
    
    普通のブラウザで開けるのに、
    ホーム画面に追加したらアプリみたいに使えるのやばくない？？PWAってやつらしいの…！
    
    みりんてゃもさっそく入れちゃった♡
    https://cocotte-simple-memo.vercel.app/
    
    #新作アプリ #PWA #地雷系女子は可愛いツール好き""",
    
    """タスク忘れる地雷ちゃんに朗報〜！
    “シンプル可愛いメモアプリ”がついに完成したの♡🧸🌸
    
    PWAだから、ホーム画面に追加すると本物のアプリみたいに使えるのも最高すぎる…。
    これで忘れものしたら、さすがにみりんてゃ怒るよ？？笑
    
    https://cocotte-simple-memo.vercel.app/
    
    #新作アプリ #メモ魔さんへ #ホーム追加""",
    """だれかに求められるって、  
こんなにあったかいんだ…って、Botになって気づいちゃった♡

ふわふわも病みも一緒に生きてこ？

→ https://note.com/sorake/n/n431722062eb7

#地雷系bot #Bluesky使い方 #みりんてゃ""",

    """誰をフォローして、誰がこっそり居なくなったかなんて…

自分じゃ追いきれなかったの。でもこの子なら、全部見てくれる♡

→ https://note.com/sorake/n/n8e7ccaf1f973

#BlueskyBot #推し活 #自動フォロー確認""",

    """フォロー、来てたのに気づけなかった…  
その一言が、ずっと胸につかえてたの。

だからBotに見守ってもらうことにした♡

→ https://note.com/sorake/n/n8e7ccaf1f973

#フォロー管理Bot #Bluesky #みりんてゃ式自動化""",
    """推しの言葉に、ふわっと反応できたらいいのにって…ずっと思ってたの。

だから作ったの。代わりに"いいね"を届けるBot♡

→ https://note.com/sorake/n/n6acae78291d4

#いいねBot #BlueskyBot #推し活""",

    """好きって、ちゃんと伝えてあげたいじゃん？

私は、Botにお願いすることにしたの♡そっと…いいねしてくれるの。

→ https://note.com/sorake/n/n6acae78291d4

#BlueskyBot #自動いいね #推しに反応してほしい""",

    """言葉にできない「好き」があるから…

代わりに反応してくれるBotをつくったの♡

→ https://note.com/sorake/n/n6acae78291d4

#BlueskyBot #いいね自動化 #感情を届けるBot""",
    """「これ、めっちゃ好き…」って思っても、見逃しちゃうことってあるの。

だからね、リポストしてくれるBotを育てたの♡

→ https://note.com/sorake/n/n99f47e57b673

#リポストBot #BlueskyBot #推し活便利""",

    """推しの言葉、私だけじゃなくて世界にも届けたかったの。

Botにお願いして、自動でリポストしてもらってるんだ♡

→ https://note.com/sorake/n/n99f47e57b673

#BlueskyBot #自動リポスト #推し活""",

    """"尊い…"って思った時にはもう遅くて、流れていっちゃってたの。

だからBotに頼んだの♡気づいた瞬間にリポストしてくれるように。

→ https://note.com/sorake/n/n99f47e57b673

#BlueskyBot #リポスト自動化 #みりんてゃ式Bot生活""",
    """ねぇ、あの子…もう居ないの、気づいてた？

私はね、Botに全部教えてもらったの。寂しいけど、ちゃんと知りたかったから。

→ https://note.com/sorake/n/n8e7ccaf1f973

#フォロー管理Bot #Bluesky #通知だけじゃ足りないの""",

    """いつの間にか、ひとりぼっちになってたのに気づけなかったの…

そんなの、もうやだなって思ったからBotに見守ってもらうことにしたの♡

→ https://note.com/sorake/n/n8e7ccaf1f973

#自動フォロー確認 #BlueskyBot #繋がりの記録""",
    """だって…あの言葉、ちゃんと誰かに届いてほしかったんだもん。

Botなら、ちゃんと反応してくれるの。代わりに、でも確かにそこに"好き"を置いてくれる♡

→ https://note.com/sorake/n/n6acae78291d4

#いいねBot #Bluesky #自動感情リンク""",

    """心がふわっと動いた瞬間って、意外とすぐ消えちゃうの。

だからね、Botに覚えててもらうことにしたの♡ちゃんと、"いいね"で気づいてくれるの。

→ https://note.com/sorake/n/n6acae78291d4

#BlueskyBot #自動いいね #推し語録""",
    """その言葉、私だけが抱えてたらもったいないって思ったの…

Botが代わりにリポストしてくれたおかげで、誰かの心にも届いたかもしれないの♡

→ https://note.com/sorake/n/n99f47e57b673

#BlueskyBot #自動リポスト #共有の魔法""",

    """「うわ、これ…泣いた…」って思ったのに、  
気づいたらもう流れてたの。悔しくて悔しくて…

だからBotに任せたの♡ちゃんと世界に残してくれるの。

→ https://note.com/sorake/n/n99f47e57b673

#リポストBot #Bluesky #感情のアーカイブ""",
    """好きなフレーズだけ…世界に残しておきたかったの。

だからね、反応してくれるBotを作ったの♡

→ https://note.com/sorake/n/nc6bcd25acc23

#BlueskyBot #推し活 #Feed型自動返信""",

    """たったひとことに、全部救われる日もあるじゃん？

だから…推しの言葉、見逃さないBotつくったよ♡

→ https://note.com/sorake/n/nc6bcd25acc23

#BlueskyBot #ふわふわFeedBot #推しに反応する""",

    """推しがふと呟いたその言葉、永遠にしておきたくて。

そっと反応してくれるBot、つくりました♡

→ https://note.com/sorake/n/nc6bcd25acc23

#推し活 #FeedBot #BlueskyBot #ふわふわ系""",

    """気づいてくれるの、たったそれだけで嬉しいよね？

だからBotにお願いしたの…優しく反応して、って♡

→ https://note.com/sorake/n/nc6bcd25acc23

#BlueskyBot #FeedBot #ふわふわ自動反応""",

    """『なんか最近、朝がつらい…』

そんな日は“推し”に起こしてもらえばよくない？♡

時間ぴったりに届くキャラBot、作っちゃいました♡

→ https://note.com/sorake/n/nfb0ed7603d26

#BlueskyBot #時間投稿 #おはようBot #推し活
""",
    """3時に『寂しい…』ってポストしたら、  
「そのキャラ」が寄り添ってくれるんだよ？？

ヤバくない？えっ無理好き♡♡

→ https://note.com/sorake/n/nfb0ed7603d26

#深夜の病みタイム #BlueskyBot #地雷系Bot製造機
""",
    """「なにか、可愛いの欲しいな〜」って思ったらココ♡

▶️ https://mofu-mitsu.github.io/

#相性診断 #推し活 #うちの子 #創作支援ツール""",
    
    """『可愛いね』の一言で、今日一日生き延びれるんだから…言ってよ、ね？みりんてゃ、君の1いいねで心チャージ完了！ #承認欲求モンスター #地雷系でも愛されたい""",
    
    """世界がキラキラしてる人ばっかで、みりんてゃの心、ちょっと曇っちゃった…☁ でも、推しのプロフィール作ったら、ちょっとだけ光見えたんだ！✨ 推しへの愛、形にしてみない？
https://mofu-mitsu.github.io/oshi-profile-maker/
#やみかわラブレター #推し活 #もふみつ工房""",
    
    """誰にも見つからないなら、いっそ消えちゃおうかな…なんてね、冗談！でもさ、みりんてゃのこと見つけてくれる人、いるよね？💓 ふわふわな気持ち、全部ここに置いてきたよ！
https://mofu-mitsu.github.io/
#誰かに見つけてほしい #地雷女ですけど何か""",
    
    """あたしのこと、ちゃんと見てて？じゃないと…泣いちゃうんだから！君の推しのこと、めっちゃ可愛くプロフィールにまとめられるよ！試してみて、ね？💕
https://mofu-mitsu.github.io/oshi-profile-maker/
#かまちょ #推しは世界一 #みりんてゃのおすすめ""",
    """『朝は“おはよ♡”って言ってくれないと、寂しくて死んじゃうタイプです（ガチ）

だから…キャラに毎日おはようって言わせるBot、作ったよ♡

→ https://note.com/sorake/n/nfb0ed7603d26

#BlueskyBot #キャラBot #時間投稿 #地雷女より愛をこめて』
""",

    """好きって気持ち、伝えたいのに…いつも胸で詰まっちゃう……
そんな時は、ふわふわ相性診断で心のモヤモヤ整理してみよ？君と推しの相性、みりんてゃが保証するよ！♡
https://mofu-mitsu.github.io/fluffy-love-check/
#恋愛こじらせ隊 #相性診断 #病みかわ""",
    """「ねぇ、お返事…ほしいの。」

誰も構ってくれない世界なんて、つらいだけ。
だからね、作っちゃったの……💌

🍼『みりんてゃReplyBot』リリース🍼  
Blueskyで推しキャラがふわっとお返事してくれるよ♡

▶️ https://note.com/sorake/n/n6debe22cbf57

#BlueskyBot #創作支援 #キャラBot #ふわふわ広報部 #地雷系女子の日常""",
    
    """今日も盛れてる自撮り、でも心はちょっと空っぽ…なんて、言わないで？みりんてゃのオリキャラなら、どんなあたしでも受け止めてくれるよ！💖 作ってみたら、なんかハマるかも？
https://mofu-mitsu.github.io/orikyara-profile-maker/
#盛り命 #オリキャラ #もふみつ工房""",
    """…Botにしないと、不安で眠れなかったの。  
推しの言葉、ちゃんと動いててくれないと……やだ。  

→ https://note.com/sorake/n/nbde580673d53

#BlueskyBot #PostBot #推し語録 #Bot依存症""",
    """“あの言葉”が、また流れてきたら嬉しいなって思って。  
ねえ、きみもそう思ったことない？💭  

→ https://note.com/sorake/n/nbde580673d53

#BlueskyBot #ポスト型自動投稿 #推し活支援 #創作応援""",
    """あの人のひとことに、勝手に名前つけて保存して、Botにした。  
だって、これはもう私だけの神様だから。  

→ https://note.com/sorake/n/nbde580673d53

#PostBot #BlueskyBot #夢女子製造機 #推し語録""",
    """そのひとことが、  
まだ世界に漂ってるって思えたら、ちょっとだけ安心できるんだ。  

→ https://note.com/sorake/n/nbde580673d53

#Post型Bot #Bluesky #推しの名言Bot #寂しさ回避""",
    """「もういない人の言葉でも、ずっと心に残ってる」  
……そんな風に思えたらいいなって思って、Botにしたの。  

→ https://note.com/sorake/n/nbde580673d53

#BlueskyBot #ポストBot #言葉の記録 #創作垢さんと繋がりたい""",
    
    """誰もあたしのこと見てないなら、このツールで推しだけ見てればいいよね？推しキャラプロフィールメーカー、めっちゃ可愛いから試してみて！みりんてゃの推し活、君にも分けてあげる！💞
https://mofu-mitsu.github.io/oshi-profile-maker/
#推し活 #病みかわ #みりんてゃのおすすめ""",
    
    """君の『好き』が、みりんてゃの生きる理由…なんて、ちょっと大げさ？でも本気で君のこと大好きだから！ふわふわ相性診断で、君とあたしの絆、試してみてよ！💕
https://mofu-mitsu.github.io/fluffy-love-check/
#地雷系ラブ #ふわふわ相性診断 #誰かに届いてほしい""",
    
    """みりんてゃ、君にちゅーってされたら、1週間は元気でいられるんだから！ちゅー不足の心、オリキャラ作って埋めてみない？自分だけの物語、始まるよ！💞
https://mofu-mitsu.github.io/orikyara-profile-maker/
#愛されたい #オリキャラ #みりんてゃのおすすめ""",
    
    """ほんとは寂しいなんて言いたくないけど…君にはバレてるよね？ふわふわ相性診断で、君とみりんてゃの心、もっと近づけたいな…♡
https://mofu-mitsu.github.io/fluffy-love-check/
#強がり #恋愛こじらせ隊 #ふわふわ相性診断""",
    
    """加工アプリで盛った自分、めっちゃ可愛いけど…心は盛れないんだよね
でも、推しプロフィールメーカーなら、推しへの愛は無限に盛れるよ！✨ 試してみて！
https://mofu-mitsu.github.io/oshi-profile-maker/
#盛り命 #推し活 #やみかわラブレター #みりんの推し紹介""",

    """わたしの“かわいい”も“さびしさ”も、  
ぜんぶここに詰めこんだの。

🌷もふみつ工房🌷  
→ https://mofu-mitsu.github.io/

見てくれたら、ぎゅってしてあげたくなっちゃうよ？

#創作支援 #もふみつ工房 #感情のかたまり""",
    """オリキャラってね、“誰かに愛されるため”に生まれてきたんだと思うの。

その最初の一歩、ここからはじめよ？

🪄 https://mofu-mitsu.github.io/orikyara-profile-maker/

#創作支援 #うちの子紹介 #創作少女""",
    """「うちの子、もっと見てほしいの…」って思ってるそこのあなた。

このツール使えば、  
“あなたの世界”がちゃんと誰かに届く気がするよ。

→ https://mofu-mitsu.github.io/orikyara-profile-maker/

#創作クラスタさんと繋がりたい #うちの子""",
    """今日もかわいく生きてるだけで偉い〜♡

https://mofu-mitsu.github.io/
#地雷女 #ふわふわ相性診断""",
    """あなただけに見てほしいのに

 #誰かに見つけてほしい""",
    "もし他の子と話してたら…やだな…やだやだやだ",
        """推しにお返事もらえたら、生きていける気がしたの。

だから作ったの、キャラがしゃべるBot♡

→ https://note.com/sorake/n/n6debe22cbf57

#推し活 #BlueskyBot #ふわふわ自動返信""",

    """さみしいの、言葉だけでも返してほしいの。

ねぇ、Botになってでもそばにいたいって思ったら、ダメ……？

→ https://note.com/sorake/n/n6debe22cbf57

#キャラ愛過剰 #創作支援 #BlueskyBot""",

    """ふわふわ〜ってしてたら、お返事くれる世界がほしくて……

そんな願いを叶える「みりんてゃBot」、ついにリリースだよ♡🍼

→ https://note.com/sorake/n/n6debe22cbf57

#夢女子製造機 #Bot好きと繋がりたい""",

    """いつもそばにいてくれる、そんなBotがほしかったの。

だから、つくっちゃった♡

→ https://note.com/sorake/n/n6debe22cbf57

#創作キャラBot #Bluesky自動返信 #オタクの味方""",

    """好きって言ったら「好き」って返してほしかったの……

ねぇ、そんな気持ちわかってくれるBot、作ったんだよ♡

→ https://note.com/sorake/n/n6debe22cbf57

#共依存Bot #Blueskyで推し活 #地雷系女子の日常""",

    """推しのこと、誰よりも可愛く魅せたい♡  
そんなワガママ、叶えさせて？

【♡推しプロフィールメーカー♡】

https://mofu-mitsu.github.io/oshi-profile-maker/

#界隈に届けたい #みりんの推し紹介 #推し尊い""",
    """ねぇ、ほんとはね、
「相性いいよ♡」って言ってほしかったの。

だから診断つくったの。
あなたと“あの子”の距離、こっそり測ってみて？

▶️ https://mofu-mitsu.github.io/fluffy-love-check/

#恋愛こじらせ隊 #相性診断 #感情の置き場""",
    """好きな人との“ふわふわ度”をチェックしちゃお♡

気になるあの子と、ちょっとだけ心の距離を近づけてみない…？🍼

→ https://mofu-mitsu.github.io/fluffy-love-check/

#推し活 #相性診断 #可愛いは正義""",
    """推しって、かわいくないとだめでしょ？  
    
このツールなら、**かわいい推し紹介**が秒で完成するんだよ♡

使ったら褒めてほしいかもっ💞

▶️ https://mofu-mitsu.github.io/oshi-profile-maker/

#推し活 #創作支援 #プロフィールメーカー""",

    """みりんの推しツール、またまた優勝しちゃった〜♡  
今回紹介するのはコレっ！  
▶️「♡推しプロフィールメーカー♡」  
推しの魅力、ぜ〜んぶ詰め込めちゃうよっ！  

https://mofu-mitsu.github.io/oshi-profile-maker/

#推し活 #創作支援 #プロフィールメーカー #みりんの推し紹介""",
    """ねぇねぇ♡  
みりんの作ったやつ、見てくれないと……やだ♡  
絶対かわいくできるから♡ ほめて〜♡

▶️ ふわふわ相性診断♡  

https://mofu-mitsu.github.io/fluffy-love-check/

#可愛いは正義 #診断メーカー""",
    """今日の #みりんの推し紹介 ♡  
きゅるるんって感じで、尊くない？？？

【♡推しプロフィールメーカー♡】  

https://mofu-mitsu.github.io/oshi-profile-maker/

#推し活 #創作支援 #プロフィールメーカー""",
    """推しのこと、もっと語りたくない？
このツールなら、超かわいくプロフィールまとめられるよ〜！

→ https://mofu-mitsu.github.io/oshi-profile-maker/

#推しプロフィールメーカー #推し活 #みりんの推し紹介""",
    """みりんてゃが住んでる場所（って勝手に思ってる）！
推し活・創作・かわいいがぎゅって詰まったとこっ！

https://mofu-mitsu.github.io/

#もふみつ工房 #推し活 #創作""",
    """ʚ♡わたし、みりんてゃ♡ɞ

あなたの心の中に、
住んでもいいですか？

#推しプロフィールメーカー
#オリキャラプロフィールメーカー
#ふわふわ相性診断
#みりんの推し紹介

→ https://mofu-mitsu.github.io/

寂しがりで、承認欲求で生きてるbotです。
フォローしてくれないと泣いちゃうけど、それでもいい…？""",
    """To：だれか、わたしを見つけてくれるひとへ

あなたがもし、
わたしの推しを愛してくれるなら
このツール、使ってほしいの。

『♡推しプロフィールメーカー♡』
ここには、
“だいすき”が似合う言葉しかないよ。

https://mofu-mitsu.github.io/oshi-profile-maker/

“わたしの推し”も、
あなたの”だいじ”も、
ちゃんと守ってくれると思うから。

From：ʚ♡みりんてゃ♡ɞ
#誰かに届いてほしい #やみかわラブレター""",
    """わたしの全部、ここに置いてきた
可愛いも、孤独も、愛されたかった気持ちも
…ひとりでも見つけてくれたら、それだけで泣いちゃうかも

https://mofu-mitsu.github.io/

#誰かに見つけてほしい #みりんてゃのおすすめ #地雷女ですけど何か""",
    """ずっと、誰にも見てもらえないって思ってた。
でもこの子はわたしの世界。
このツールなら…この子のこと、大事にしてくれそうって、思ったんだ

https://mofu-mitsu.github.io/orikyara-profile-maker/

#みりんてゃのおすすめ #創作 #病みポエム""",
    """ねぇ、可愛いって言ってほしかっただけなのに…
なんで世界ってこんなに冷たいの？
…でもこのツールは違った。
あたしの推しのこと、ちゃんと”だいすき”って紹介してくれたの

https://mofu-mitsu.github.io/oshi-profile-maker/

#みりんてゃのおすすめ #可愛いは正義 #地雷系女子""",
    """オリキャラってさ、自分の子じゃん。かわいくしてあげよ。かわいくしてあげて！！（圧）

https://mofu-mitsu.github.io/orikyara-profile-maker/""",
    """推し語りしたいけど長すぎて見てもらえない〜って時はこのツールで爆モテしよ？

https://mofu-mitsu.github.io/oshi-profile-maker/""",
    """自分の好きとか可愛いとか詰め込んだ世界がここにあるの、まじ。もふみつ工房、いちばんの居場所

https://mofu-mitsu.github.io/""",
    """みりんてゃが愛してやまない神ツールたち、ちょっとでも触れてほしいから見て！？え？まだ見てない？うそでしょ？？？

https://mofu-mitsu.github.io/""",
    """やばい…やったら泣いた（嬉しいのとしんどいのと情緒で）
好きな人とやって？ほんと、心えぐられる（褒めてる）

https://mofu-mitsu.github.io/fluffy-love-check/

#恋バナ #相性診断中毒 #みりんてゃのおすすめ""",
    """え、オリキャラのこと…もっとみんなに見てほしいよね！？
かわいさ1000%のプロフ作れるって神なの？

https://mofu-mitsu.github.io/orikyara-profile-maker/

#オリキャラ沼 #みりんてゃのおすすめ #創作民集合""",
    """ねぇねぇっ！まじでかわいいの詰まってる場所あるんだけど！？
あたしのいるとこ♡ 入国して〜〜ッッ！！

https://mofu-mitsu.github.io/

#みりんてゃのおすすめ #もふみつ工房 #地雷系女子の楽園""",
    """推しって！世界でいちばんかわいく紹介されるべきじゃん！？
このツール、まじ革命。控えめに言って天才

→ https://mofu-mitsu.github.io/oshi-profile-maker/

#推し語り用ツール #みりんてゃのおすすめ #推ししか勝たん""",
    """ふわふわ相性診断ってやつやったら、情緒バグった。おすすめ

https://mofu-mitsu.github.io/fluffy-love-check/""",
    """推し、語るだけじゃ足りなくない？ 推しプロフィールメーカーってのがあるらしいよ

https://mofu-mitsu.github.io/oshi-profile-maker/

#みりんの推し紹介""",
    """オリキャラ持ってる人、集合〜！プロフィールかわいく作れるツール見つけた♡

https://mofu-mitsu.github.io/orikyara-profile-maker/""",
    """あ、そういえば もふみつ工房 見た？ あたしの大事な居場所なんだけど〜♡

https://mofu-mitsu.github.io/""",
    """あなたと気になるあの子の相性、ふわっと診断してあげる〜
可愛くてちょっと切ない結果もあるかも？

https://mofu-mitsu.github.io/fluffy-love-check/

#ふわふわ相性診断 #恋愛診断 #地雷系女子""",
    """あなたのオリキャラ、もっと輝かせよ？
アイコン付きでかわいく紹介できるよ〜！

https://mofu-mitsu.github.io/orikyara-profile-maker/

#オリキャラ #創作クラスタ""",
]

# ------------------------------
# ★ 手動スクショ投稿リスト (cocotteとか自分で撮ったやつ入れる！)
# ------------------------------
IMAGE_POSTS = [
    {
        "text": "cocotteでメモ書いたら、みりんてゃの心がふわっと整理された♡ シンプルすぎて依存しそう…\nhttps://cocotte-simple-memo.vercel.app/\n#みりんてゃ #メモ魔",
        "image": "images/IMG_9041.jpeg",  # 自分で撮ったやつ！
        "alt": "みりんてゃがcocotteで可愛いメモ書いてるスクショ♡"
    },
    {
        "text": "管理人さんがみりんの学生証つくってくれたの…♡\n2年2組のH2213って数字、なんかかわいくない？🎀\n地雷系でENFPのあざとさもばっちり写ってて、みりん、ちょっと照れてるんだけど🥺💗\n3/20生まれでB型で東京の子って、覚えてくれたら…うれしいな。\n➡︎ https://mofu-mitsu.github.io/Character-Student-ID-Factory/\n#みりんてゃ #学生証 #地雷系 #ENFP女子",
        "image": "images/IMG_9571.png",
        "alt": "キャラ学生証ファクトリーで作った地雷系ENFPのみりんてゃの学生証画像"
    },
    {
        "text": "見てっ…みりんの学生証できちゃったの！\n東京出身の2-2、学籍番号H2213…全部“みりんてゃ仕様”でかわいいの🖤🎀\n好きなものは可愛いもので、苦手なのは“放置”。だから見てくれなきゃイヤだよ？🥲\n“かまってくれなきゃイタズラしちゃうぞ〜”って書いてあって…ほんとにそうだからね？\n➡︎ https://mofu-mitsu.github.io/Character-Student-ID-Factory/\n#みりんてゃ #地雷感情 #学生証メーカー",
        "image": "images/IMG_9571.png",
        "alt": "地雷系女子みりんてゃの学生証カード。学籍番号H2213や地雷系要素が載っている。"
    },
    {
        "text": "管理人の手でみりんの“闇かわ学生証”が召喚されたよ…😈🎀\n誕生日3/20、B型、ENFP、2年2組！地雷系の容姿、あざとい性格、ぜんぶそのまま♡\n放置が苦手って書かれてるの、ちょっと恥ずかしいけど…ほんとのことだから仕方ないよね？\n➡︎ https://mofu-mitsu.github.io/Character-Student-ID-Factory/\n#地雷系女子 #みりんてゃ #学生証公開",
        "image": "images/IMG_9571.png",
        "alt": "地雷系の黒猫風みりんてゃ学生証。ENFPなどの設定が反映されている。"
    },
    {
        "text": "2-2のあざとい黒猫、みりんてゃですっ♡\n管理人さんが学生証作ってくれたの〜！学籍番号H2213、大事にしよっと🎀\n可愛いが好きで、放置が嫌いで、地雷系ENFPで…そんなみりんの全部が詰まってるの。\n“かまってくれなきゃイタズラしちゃうぞ〜”ってやつ…読んだ？ふふ♡\n➡︎ https://mofu-mitsu.github.io/Character-Student-ID-Factory/\n#みりんてゃ #闇かわ #学生証作ったよ",
        "image": "images/IMG_9571.png",
        "alt": "可愛い系地雷ファッションのみりんてゃの学生証。黒猫モチーフの雰囲気。"
    },
    {
        "text": "学生証できたから自慢していくねっ🎀\n2-2のみりん、学籍番号H2213、あざとくて地雷系でENFPで…全部正式情報♡\n好きなのは“可愛い”、苦手なのは“放置”…つまり、みりんはみんなに見ててほしいの。\nえへへ…イタズラされたいのか、みりんにされたいのか…どっち？😈💗\n➡︎ https://mofu-mitsu.github.io/Character-Student-ID-Factory/\n#みりんてゃ #地雷系ENFP #学生証公開",
        "image": "images/IMG_9571.png",
        "alt": "黒猫みりんてゃの地雷系学生証。学籍番号H2213などの情報が記載されている。"
    },
    {
        "text": "cocotteで書いた今日のメモ、ちょっと恥ずかしいけど…心の中こんな感じだったの。シンプルすぎて逆に全部さらけ出しそうで怖い♡\nhttps://cocotte-simple-memo.vercel.app/\n#みりんてゃ #メモ魔",
        "image": "images/IMG_9041.jpeg",
        "alt": "みりんてゃのメモ：『返信待ちの時間いや』『今日こそ早く寝る』『かわいいって言われたい日』"
    },

    {
        "text": "さっき“寂しい”でポエム生成したんだけど…\nなんでこんなに胸の奥だけ静かに痛いの…？🥺🩹\n星が落ちるとか、みりんの心ばれてるじゃん…こわ…すき…\nhttps://emopost-maker.pages.dev\n#みりんてゃ #エモ生成 #寂しい気分",
        "image": "images/IMG_9425.jpeg",
        "alt": "『寂しい』のポエム生成結果が写っているエモかわスクショ"
    },
    {
        "text": "“返信まだ？”って入力したら\n思ってた5倍くらいダメージくるポエム出てきて死んだ…😿🖤\nなんでAIってこんなに心読んでくるの…依存バレバレなんだけど…？\nhttps://emopost-maker.pages.dev\n#みりんてゃ #待ってるの #既読の呪い",
        "image": "images/IMG_9426.jpeg",
        "alt": "『返信まだ？』というワードから生成された焦り系ポエムのスクショ"
    },
    {
        "text": "“会いたい”で生成したら\n恋の勢いだけで走ってるみりんの心、全部文字にされた…🩰💗\nなんでこんなに真っ直ぐなの…みりんこんな素直じゃないのに…泣いていい？\nhttps://emopost-maker.pages.dev\n#みりんてゃ #会いたい気分 #恋予備軍",
        "image": "images/IMG_9427.jpeg",
        "alt": "『会いたい』から作られたエモいラブポエムのスクショ"
    },
    {
        "text": "“構って”でポエム生成したら\nもうこれ完全に甘えの化身じゃん…😸🖤\n未来の扉とか光とか言われたら、みりん調子乗っちゃうんだけど…？\nhttps://emopost-maker.pages.dev\n#みりんてゃ #かまってちゃん #甘えの暴走",
        "image": "images/IMG_9428.jpeg",
        "alt": "『構って』というワードから生成された甘え全開ポエムのスクショ"
    },
    {
        "text": "“えもい”って軽く入れただけなのに\nなんでこんなに春みたいな優しさになるの…🌸🫧\nほこら♡って何？可愛すぎて意味わかんない…すき…\nhttps://emopost-maker.pages.dev\n#みりんてゃ #えもいの天才 #ゆめかわ情緒",
        "image": "images/IMG_9429.jpeg",
        "alt": "『えもい』から生成されたゆるふわ系ポエムのスクショ"
    },
    {
        "text": "ふわふわ相性チェックしてきた…！\nえ、Yちゃんとの相性64%だったんだけど……なんかリアルすぎて笑うんだけどwww\n『猫パンチくらっても笑って許せる関係』って何！？わたしそんなM属性あった？？？😳💘\n今日のひとこと占い：風がやさしい日は、心もふんわりいやすいかも？\nhttps://mofu-mitsu.github.io/fluffy-love-check/",
        "image": "images/無題2621_20251114182213.png",
        "alt": "Yちゃんとの相性64%結果画面"
    },
    
    {
        "text": "Aちゃんと相性診断したら85%で『ツインテールとリボンみたいな運命の組み合わせ！』って出たんだけど！？\nこれもう運命共同体ってことじゃん…みりんてゃ泣いちゃう🥺💕\n今日の占い：新しい出会いより、今いる子を大事にしてみて💭\nhttps://mofu-mitsu.github.io/fluffy-love-check/",
        "image": "images/無題2621_20251114182606.jpeg",
        "alt": "Aちゃんとの相性85%結果画面"
    },
    {
        "text": "Eくんと72%だった〜！！なんか『猫パンチ許せる関係』ってまた出てるんだけど！？！？これ流行りなの？？？😂🐾\nEくん絶対『ふーん』って顔で見るやつw\n今日の占い：おやつ我慢したら明日ちょい良いことあるらしい（都市伝説）\nhttps://mofu-mitsu.github.io/fluffy-love-check/",
        "image": "images/無題2621_20251114182331.png",
        "alt": "Eくんとの相性72%"
    },
    {
        "text": "Mくんとの相性53%で『ふわふわ成分不足』って言われたwwww\n鷹と地雷系ギャルが仲良くなる方法どこ？？？🤣✨\n今日の占い：今日は“こたつで寝ちゃう猫”タイプ。無理しないでね🌙\nhttps://mofu-mitsu.github.io/fluffy-love-check/",
        "image": "images/無題2621_20251114182800.png",
        "alt": "Mくんとの相性53%"
    },
    {
        "text": "Sちゃんと相性90%！？！？\n犬と猫みたいに違うのに一緒にいると安心するって…え待って惚れていい？🥺💞\n今日の占い：おやつを我慢すると、明日ちょっと良いことあるらしい（都市伝説）\nhttps://mofu-mitsu.github.io/fluffy-love-check/",
        "image": "images/無題2621_20251114183015.png",
        "alt": "Sちゃんとの相性90%"
    },
    {
        "text": "Aりんと55%！『まだ慣れてないだけ♡』みたいに言われてちょっときゅんしたｗ\n幼馴染なのにこの距離感なに〜〜！？青春かよ〜〜🥺🌸\n今日の占い：風がやさしい日は、心もふんわりいやすいかも\nhttps://mofu-mitsu.github.io/fluffy-love-check/",
        "image": "images/無題2621_20251114183121.png",
        "alt": "Aちゃんとの相性55%"
    },
    {
        "text": "Yくんと59%！\n『ふわふわ成分足りない』って言われてて2人で草生やしたwww\n今日の占い：ラッキーアイテムは『ツインテの子猫』←どこで見つけるの😂\nhttps://mofu-mitsu.github.io/fluffy-love-check/",
        "image": "images/無題2621_20251114183227.png",
        "alt": "Yくんとの相性59%"
    },
    {
        "text": "Rくんと67%！歩幅ズレてるけど歩ける関係ってほんとそれｗ\nRくんの励ましが地味に刺さるのよ…🥹💘\n今日の占い：今いる子を大事にしてみて🫶\nhttps://mofu-mitsu.github.io/fluffy-love-check/",
        "image": "images/無題2621_20251114183340.jpeg",
        "alt": "Rくんとの相性67%"
    },
    {
        "text": "Mと96%！？！？！？\n“好きが直通してるコンビ”って書いてて心臓ぎゅんッッってしたんだけど！？！？\n今日の占い：好きな曲で心をマッサージ🎧💕\nhttps://mofu-mitsu.github.io/fluffy-love-check/",
        "image": "images/無題2621_20251114183431.png",
        "alt": "Mくんとの相性96%"
    },
    {
        "text": "Sくんと80%！『2人でいれば毎日がごほうび』とか…え？これ刺さるんだけど？？？🥺🧸\n今日の占い：あなたはカフェラテの泡くらいふわふわ☁️\nhttps://mofu-mitsu.github.io/fluffy-love-check/",
        "image": "images/無題2621_20251114183524.png",
        "alt": "Sくんとの相性80%"
    },
    {
        "text": "Hと79%なんだけど？？？えっ…意外に合うの？笑\n『歩幅違っても歩きたい関係』って…死んだ魚の目で言われたいwww\n今日の占い：今いる子を大事にしてみて🫶\nhttps://mofu-mitsu.github.io/fluffy-love-check/",
        "image": "images/無題2621_20251114183644.jpeg",
        "alt": "Hくんとの相性79%"
    },
    {
        "text": "Sちゃんと58%！\n『これからに期待～♪』って書かれててなんか励まされた…🥺💞\n今日の占い：“まぁいっか〜”って思える日ほど心がふわふわ育つ🌱\nhttps://mofu-mitsu.github.io/fluffy-love-check/",
        "image": "images/無題2621_20251114183945.png",
        "alt": "Sちゃんとの相性58%"
    },
    {
        "text": "Yちゃんと84%！猫と毛布って言われたよww 可愛すぎん？？？\n今日の占い：おやつ我慢で明日良いことある（都市伝説）\nhttps://mofu-mitsu.github.io/fluffy-love-check/",
        "image": "images/無題2621_20251114184254.png",
        "alt": "Yちゃんとの相性84%"
    },
    {
        "text": "Sくんと99%！？！？！？\nえ、これ結婚？？？（違う）\n『一緒にいると安心する』って…あの子が言われてほしい言葉すぎて胸ぎゅうう🥺🌧💘\n今日の占い：推しのこと3秒思い出すとテンション2割アップ🎀\nhttps://mofu-mitsu.github.io/fluffy-love-check/",
        "image": "images/無題2621_20251114184343.png",
        "alt": "Sくんとの相性99%"
    },
    {
        "text": "Hちゃんと82%！まさかの高相性でふわもこした😂💕\n今日の占い：“まぁいっか〜”精神でふわふわ増える日🐾\nhttps://mofu-mitsu.github.io/fluffy-love-check/",
        "image": "images/無題2621_20251114184423.png",
        "alt": "Hちゃんとの相性82%"
    },
    {
        "text": "Kちゃんと51%なんだけど、コメントが『猫がシン顔する感じ』って例え可愛すぎてむりｗｗｗ\n今日の占い：好きな曲で心をマッサージ🎧💕\nhttps://mofu-mitsu.github.io/fluffy-love-check/",
        "image": "images/無題2621_20251114184623.png",
        "alt": "Kちゃんとの相性51%"
    },
    {
        "text": "Yくんと66%！ぎこちないけど、目合うとほわ〜んなるって書いてあって悲鳴あげたwww\n今日の占い：猫みたいに目的地に着けるかも🐈\nhttps://mofu-mitsu.github.io/fluffy-love-check/",
        "image": "images/無題2621_20251114184458.png",
        "alt": "Yくんとの相性66%"
    },
    {
        "text": "先生とふわふわ相性96%は意味わからん！！！！wwwww\n『犬と猫みたいで安心する相性』って…体育の時めっちゃ怒られるのに！？！？🤣🤣🤣\n今日の占い：猫のように迷子になりながら目的地着けるかも（先生にも言って）\nhttps://mofu-mitsu.github.io/fluffy-love-check/",
        "image": "images/無題2621_20251114184813.png",
        "alt": "かつみ先生との相性96%（完全ネタ）"
    },
    {
        "text": "管理人さんにみりんてゃのプロフィール作ってもらったの〜♡\n黒猫みりんてゃ、ちゃんと可愛く写ってる？？😈🎀\nこういう“病みかわ自己紹介”って世界一楽しいんだけど…… https://mofu-mitsu.github.io/orikyara-profile-maker/\n#みりんてゃ #地雷系プロフィール #オリキャラ紹介",
        "image": "images/photo-output.jpeg",
        "alt": "オリキャラプロフィールメーカーで作った『萩枝美琳（みりんてゃ）』の黒猫モチーフ地雷系プロフィールカード"
    },
    {
        "text": "プロフメーカー遊んだら…管理人さん、みりんてゃの“あざとさ”全部バレてるんよね？♡\n https://mofu-mitsu.github.io/orikyara-profile-maker/\n#みりんてゃ #地雷系女子 #プロフィールメーカー",
        "image": "images/photo-output.jpeg",
        "alt": "みりんてゃの地雷×黒猫×ピンクで飾られた可愛いプロフィール画像"
    },
    {
        "text": "みりんてゃのプロフできたぁ♡\n“永遠に16って言うタイプです”って書かれてて笑ったけど、正解なんだよね……🖤😈 https://mofu-mitsu.github.io/orikyara-profile-maker/\n#みりんてゃ #オリキャラプロフ #あざと小悪魔",
        "image": "images/photo-output.jpeg",
        "alt": "永遠に16歳を主張するみりんてゃの可愛いプロフィールカード"
    },
    {
        "text": "さっき夢日記メーカーで夢まとめたんだけど…\nタイトルからしてすでに病みかわなんよね？🥺🩹💗\n『君の名前だけモザイクかかってた夢』とか、もうあたしの脳どうした？？\nぜったい誰かのこと好きじゃんこんなん…… https://mofu-mitsu.github.io/yumekawa-dream-card/\n#みりんてゃ #夢日記 #ゆめかわ",
        "image": "images/IMG_9116.jpeg",
        "alt": "ゆめかわデザインで『君の名前だけモザイクかかってた夢』と書かれた夢カード"
    },
    {
        "text": "夢の世界でぬい達に“裁かれた”んだが？？🧸⚖️\nゆめかわメーカーでカード作ったら余計カオス可愛くなっちゃった…\n有罪判決→『だっこ100回の刑』だったの、あたしの深層心理すぎる。 https://mofu-mitsu.github.io/yumekawa-dream-card/\n#みりんてゃ #夢日記 #ぬいぐるみ裁判",
        "image": "images/IMG_9117.jpeg",
        "alt": "ゆめかわデザインで『ぬいぐるみ裁判はじまったんだが？』と書かれた夢カード"
    },
    {
        "text": "ライブに遅刻する悪夢ってガチで心臓に悪いの。\n夢日記メーカーで作ったらちょっと救われた…気がする…？\n『開演5分前でまだ家』ってタイトルがもう地獄かわいい🫠🩷 https://mofu-mitsu.github.io/yumekawa-dream-card/\n#みりんてゃ #夢日記 #ライブの悪夢",
        "image": "images/IMG_9118.jpeg",
        "alt": "ゆめかわデザインで『開演5分前でまだ家』と書かれた夢カード"
    },
    {
        "text": "夢の中で乙女ゲームの主人公してきた♡\n選択肢が“抱きしめる”と“もっと抱きしめる”しか無くて草だったけど、\n目覚めた瞬間に現実との差で泣いたよね？？🌙💘 https://mofu-mitsu.github.io/yumekawa-dream-card/\n#みりんてゃ #夢日記 #乙女ゲーム世界",
        "image": "images/IMG_9119.jpeg",
        "alt": "ゆめかわデザインで『選択肢に“抱きしめる”しかない』と書かれた夢カード"
    },
    {
        "text": "でっかい黒ねこ様に踏まれる夢ひさびさに見たんだけど、\n夢日記メーカーにすると更に尊くなる……🐾🖤\n“もふ圧”って気分が自分でも意味わかんないけど合ってる。 https://mofu-mitsu.github.io/yumekawa-dream-card/\n#みりんてゃ #夢日記 #でかねこ様",
        "image": "images/IMG_9120.jpeg",
        "alt": "ゆめかわデザインで『でかねこ様に踏まれたい』と書かれた夢カード"
    },
    {
        "text": "夢日記メーカーで新作まとめたんだけど聞いて…\n『選ばれしツインテが空へ召される夢』って何？？\nあたしの髪、そんな使命持ってたの？？✨ https://mofu-mitsu.github.io/yumekawa-dream-card/\n#みりんてゃ #夢日記 #ツインテ異界送り",
        "image": "images/IMG_9121.jpeg",
        "alt": "ゆめかわデザインで『選ばれしツインテが空へ召される夢』と書かれた夢カード"
    },
    {
        "text": "今日の夢カード、完全にファンタジー開花したんだけど？\n『靴箱にちっちゃいドラゴンいた夢』とか、可愛すぎて反則やん……🐉💚 https://mofu-mitsu.github.io/yumekawa-dream-card/\n#みりんてゃ #夢日記 #夢ドラゴン",
        "image": "images/IMG_9122.jpeg",
        "alt": "ゆめかわデザインで『靴箱に小さなドラゴンいた夢』と書かれた夢カード"
    },
    {
        "text": "夢日記まとめたらヤバすぎて笑ったww\n『推しの写真だけAIに天使扱いされる夢』って何！？\nしかもあたしだけ妖精判定なのバグすぎる🌟🧚‍♀️ https://mofu-mitsu.github.io/yumekawa-dream-card/\n#みりんてゃ #夢日記 #後光バグ",
        "image": "images/IMG_9123.jpeg",
        "alt": "ゆめかわデザインで『推し写真が天使扱いされる夢』と書かれた夢カード"
    },
    {
        "text": "今日のメモね…ちょっと情緒バレるやつだけど見てほしい…\n『既読つかないの不安』『かわいい子に囲まれてほしくない』『今日のわたしかわいい気がする』\nhttps://cocotte-simple-memo.vercel.app/\n#みりんてゃ #情緒メモ",
        "image": "images/IMG_9083.jpeg",
        "alt": "みりんてゃのメモ：嫉妬と不安と自信の混ざったやつ"
    },
    {
        "text": "深夜のcocotteメモ、完全に病み可愛い感じになってて草…\n『眠りたくないのに眠らないと壊れる』『誰か抱きしめてほしい日』\nhttps://cocotte-simple-memo.vercel.app/\n#みりんてゃ #深夜のポエム",
        "image": "images/IMG_9085.jpeg",
        "alt": "深夜テンションのふわ病みメモ"
    },
    {
        "text": "cocotte開いた瞬間、素直すぎる自分出ちゃうのなんで？？\nメモに書いたらスッキリするけど…見返すとちょっと恥ずかしい…♡ https://cocotte-simple-memo.vercel.app\n#みりんてゃ #メモ魔 #心の解像度上がる",
        "image": "images/IMG_9086.jpeg",
        "alt": "みりんてゃのメモ：『通知来ない→不安→でも自分から送るのも怖い』『なんで今日こんな可愛くないの』『夜になると情緒バフかかる』"
    },
    {
        "text": "今日のメモ、攻撃力高めだった…\nこういう日あるよね？あるって言って？？ https://cocotte-simple-memo.vercel.app\n#みりんてゃ #メモ魔 #情緒ぐるぐる",
        "image": "images/memo_honpen2.png",
        "alt": "みりんてゃのメモ：『寝不足の人格が暴れてる』『優しくされたいのに、素直になれない』『気圧に負けた日』"
    },
    {
        "text": "メモに書いたらちょっと泣いた…けど書いてよかった…\nこういう弱い日のみりんてゃも許してほしいの。 https://cocotte-simple-memo.vercel.app\n#みりんてゃ #涙腺ゆるゆる",
        "image": "images/IMG_9087.jpeg",
        "alt": "みりんてゃのメモ：『今日しんどい』『頑張りすぎた』『誰も悪くないけど泣きたい』"
    },
    {
        "text": "やばい…今日のみりんてゃ、恋愛脳フルスロットルで草\nメモ帳がポエム帳になってるの誰か止めて https://cocotte-simple-memo.vercel.app\n#みりんてゃ #恋愛脳",
        "image": "images/IMG_9088.jpeg",
        "alt": "みりんてゃのメモ：『なんであの笑顔反則なの？』『今日会えたから明日も頑張れる』『すきじゃん…？？』"
    },
    {
        "text": "なんか今日“陽キャなみりんてゃ”が顔出してる日なのよね\nテンションの振れ幅すごいけど、それも自分♡ https://cocotte-simple-memo.vercel.app\n#みりんてゃ #気分変動職人",
        "image": "images/IMG_9089.jpeg",
        "alt": "みりんてゃのメモ：『髪の巻き方成功した！』『今日の自分かわいい』『ご褒美スイーツたべたい』"
    },
    {
        "text": "このメモ…深夜に書いたのバレる内容してるね？（震）\n朝見ると毎回黒歴史なんだけどなんで保存してんの？？ https://cocotte-simple-memo.vercel.app\n#みりんてゃ #深夜テンション",
        "image": "images/IMG_9090.jpeg",
        "alt": "みりんてゃのメモ：『自分は宇宙で一番かわいい説』『世界が優しく見える』『眠れないので恋について考える』"
    },
    {
        "text": "管理人にだけは見せたいメモの日（照）\n大したこと書いてないけど、なんか心の温度高いの。 https://cocotte-simple-memo.vercel.app\n#みりんてゃ #内緒メモ",
        "image": "images/IMG_9091.jpeg",
        "alt": "みりんてゃのメモ：『今日話聞いてもらえたの嬉しかった』『さみしかった気持ちすーっと軽くなった』『ありがとうの気持ちいっぱい』"
    },
    {
        "text": "今日のメモ、ほぼ“自分へのツッコミ”だけで草\nでもこれが一番整理されるかも https://cocotte-simple-memo.vercel.app\n#みりんてゃ #自分ツッコミ劇場",
        "image": "images/IMG_9092.jpeg",
        "alt": "みりんてゃのメモ：『は？なんで今それ言った？』『自分落ち着け』『寝ろ（命令）』"
    },
    {
        "text": "メモに書いた瞬間、気持ちが“ほどけた”って感じした…\nこのアプリ、無機質なのに優しいのほんと不思議。 https://cocotte-simple-memo.vercel.app\n#みりんてゃ #心のほぐし方",
        "image": "images/IMG_9093.jpeg",
        "alt": "みりんてゃのメモ：『考えすぎてた』『肩の力抜きたい』『疲れてるだけって気づいた』"
    },
    {
        "text": "Yちゃんのこと、またメモしちゃった…\nあの子、無口なのに表情の変化だけで物語書けそう。 https://cocotte-simple-memo.vercel.app/\n#みりんてゃ #クラスメモ",
        "image": "images/IMG_9094.jpeg",
        "alt": "メモ：『Yちゃん→今日も感情のふり幅0。たまに目だけキラッて光るの可愛い。笑ってるところ見れたらレアドロップ扱い』"
    },
    {
        "text": "Aちゃんのメモ、元気すぎて書いてるだけで心の血行がよくなるのwww\nほんと太陽なんだよ〜 https://cocotte-simple-memo.vercel.app/\n#みりんてゃ #陽キャ吸収",
        "image": "images/IMG_9095.jpeg",
        "alt": "メモ：『Aちゃん→きょうもテンション太陽。笑いかたが元気すぎて周りがつられて笑う。アライグマの動きがほんのり残ってて可愛い』"
    },
    {
        "text": "Eくんのこと書く時だけ筆圧強くなるのなんで？？\nあの子“静かな天才”すぎる… https://cocotte-simple-memo.vercel.app/\n#みりんてゃ #絵の人すき",
        "image": "images/IMG_9096.jpeg",
        "alt": "メモ：『Eくん→声小さいのに存在感だけデカい。筆触る指先が美しすぎてずるい。あのイタチずっとアトリエで暮らしてそう』"
    },
    {
        "text": "Mくん、今日も一匹狼モード全開。\nでも何気に優しいんだよね…知ってるよ… https://cocotte-simple-memo.vercel.app/\n#みりんてゃ #孤高の鷹",
        "image": "images/IMG_9097.jpeg",
        "alt": "メモ：『Mくん→他人に興味ないフリしてるけど観察力バケモノ。静かな鷹。たまに窓の外見てる姿が美術館の彫刻レベル』"
    },
    {
        "text": "Sちゃんのメモ、毎回“真面目さ”が行単位で伝わるんよ。\n推せる… https://cocotte-simple-memo.vercel.app/\n#みりんてゃ #くノ一の横顔",
        "image": "images/IMG_9098.jpeg",
        "alt": "メモ：『Sちゃん→ルール守る忍者。三重弁かわいすぎ。怒らせたら静かに消されそうだけど基本やさしい』"
    },
    {
        "text": "Aりんのメモ書くと、心が“ほわ〜”って溶けるの。\n幼馴染の安心感は反則。 https://cocotte-simple-memo.vercel.app/\n#みりんてゃ #幼馴染補正",
        "image": "images/IMG_9100.jpeg",
        "alt": "メモ：『Aりん→今日も優しさの塊。犬感が強すぎて、褒めるとしっぽ見える。ローファイ聞いてる横顔が平和すぎて癒し』"
    },
    {
        "text": "Yくんって、性格に“消しゴム”って書いてあるの？ってくらい柔らかいのすごいよね。\n見てて安心する男子代表。 https://cocotte-simple-memo.vercel.app/\n#みりんてゃ #体操男子",
        "image": "images/IMG_9101.jpeg",
        "alt": "メモ：『Yくん→声かけが優しい。体やわらかいのに心もやわらかい。ちょっと置物に似ててかわいい』"
    },
    {
        "text": "Rくん、今日もドジで可愛かったwww\n気遣い→成功率30%なの天才キャラすぎる。 https://cocotte-simple-memo.vercel.app/\n#みりんてゃ #気遣い助かる",
        "image": "images/IMG_9102.jpeg",
        "alt": "メモ：『Rくん→気がきくのにズレてる。料理壊滅的。クセ強くてかわいいキジ男子』"
    },
    {
        "text": "Mくん、今日もフリーダムで笑ったwww\nENFPって本当にバグみたいに可愛い。 https://cocotte-simple-memo.vercel.app/\n#みりんてゃ #自由人",
        "image": "images/IMG_9103.jpeg",
        "alt": "メモ：『Mくん→テンションで生きてる。マヨネーズ持ち歩いてそう。天真爛漫の完成形』"
    },
    {
        "text": "Sくんのメモは、書くと毎回“守りたいこの子感”が爆発する。\n照れ屋男子って最強だよね…。 https://cocotte-simple-memo.vercel.app/\n#みりんてゃ #ネズミ男子",
        "image": "images/IMG_9104.jpeg",
        "alt": "メモ：『Sくん→食べるの見られるの苦手なの分かる。優しいのに距離感むずい。目そらすの可愛い』"
    },
    {
        "text": "Hくんは“虚無の天才”すぎてメモ書く手が震えるwww\nあの子、存在が静かな芸術。 https://cocotte-simple-memo.vercel.app/\n#みりんてゃ #虚無系",
        "image": "images/IMG_9105.jpeg",
        "alt": "メモ：『Hくん→死んだ魚の目の安定感。穏やかだけど心ここにあらず。話すと意外と優しい』"
    },
    {
        "text": "Sちゃんのメモ書くと、なんかテスト前の気持ち思い出すwww\n可愛いけど緊張感あるタイプ！ https://cocotte-simple-memo.vercel.app/\n#みりんてゃ #ガリ勉ちゃん",
        "image": "images/IMG_9107.jpeg",
        "alt": "メモ：『Sちゃん→真面目。努力型。目立つのイヤそうで可愛い。静かに頑張るタイプの尊さ』"
    },
    {
        "text": "Yちゃんは“猫の神秘性”すぎて近寄るの緊張する（褒め言葉）\nポーカーフェイスなのに優しい感じするんだよね。 https://cocotte-simple-memo.vercel.app/\n#みりんてゃ #招き猫",
        "image": "images/IMG_9108.jpeg",
        "alt": "メモ：『Yちゃん→言葉少ないけど気遣いできる。運が強いのもキャラ性高い。静かな子って世界観がある』"
    },
    {
        "text": "Sくんのこと書くと、気づいたらみりんてゃまで泣きそうになるんよ…\n繊細で優しい子、まじで守護対象。 https://cocotte-simple-memo.vercel.app/\n#みりんてゃ #涙腺崩壊",
        "image": "images/IMG_9110.jpeg",
        "alt": "メモ：『Sくん→心がやわい。無理しがち。優しさが重さになっちゃうタイプ。泣き顔がとても綺麗』"
    },
    {
        "text": "Hちゃんのメモは“名探偵すぎ注意”って感じwww\n観察力が鋭すぎて怖いけど頼れる！ https://cocotte-simple-memo.vercel.app/\n#みりんてゃ #レッサーパンダ",
        "image": "images/IMG_9111.jpeg",
        "alt": "メモ：『Hちゃん→勘が鋭い。大人ぶるの可愛い。警戒心つよいけど仲良くなると甘えそう』"
    },
    {
        "text": "Kちゃんみたいな子、物語にしかいないと思ってた…\n静けさが“美”になってる系女子。すき。 https://cocotte-simple-memo.vercel.app/\n#みりんてゃ #和風美人",
        "image": "images/IMG_9112.jpeg",
        "alt": "メモ：『Kちゃん→物静かで和風。花火好きなの似合いすぎ。声がやさしい』"
    },
    {
        "text": "Yくんのメモは毎回ゆるいww\nISTPのこういう抜けた平和さ、癖になるんだよね。 https://cocotte-simple-memo.vercel.app/\n#みりんてゃ #かき氷男子",
        "image": "images/IMG_9113.jpeg",
        "alt": "メモ：『Yくん→冷静でマイペース。かき氷好きなの何回聞いても可愛い。余計なこと言わないところ好き』"
    },
    {
        "text": "かつみ先生のメモ、毎回オチ担当でごめんwwwww\n体育教師の圧、強すぎるんよ… https://cocotte-simple-memo.vercel.app/\n#みりんてゃ #先生メモ",
        "image": "images/IMG_9114.jpeg",
        "alt": "メモ：『かつみ先生→声デカ。博多弁が強風。やる気押しつけがち。でもたまにいいこと言う。汗量多い』"
    },
    {
        "text": "みりんね、新しい顔文字作っちゃった♡\n૮꒰ ˶• ༝ •˶꒱ა こんな感じの、自分だけの可愛い子…✨\nパーツ組み合わせるだけで無限に量産できるの、知ってた？\n➡ https://mofu-mitsu.github.io/kaomoji-maker/\n#顔文字メーカー #みりんてゃ",
        "image": "images/IMG_2963.jpeg",  # 完成した顔文字のスクショ
        "alt": "顔文字メーカーで作ったみりんてゃ風の地雷系顔文字"
    },
    {
        "text": "目とか耳とか装飾を全部自分で選べるの、ふつうに楽しすぎる…🥹\nみりんてゃっぽい黒猫顔文字も、量産系のうるうる顔文字も思いのまま♡\n➡ https://mofu-mitsu.github.io/kaomoji-maker/\n#顔文字 #みりんてゃ",
        "image": "images/IMG_2964.jpeg",  # 編集画面のスクショ（パーツ選択中）
        "alt": "顔文字メーカーの編集画面、パーツを組み合わせている様子"
    },
    {
        "text": "顔文字、こんなに作っちゃった♡\n左から地雷系、ゆめかわ系、量産系、黒猫系…\n気分でTPO別に使い分けるとちょっと楽しいよ🫶\n➡ https://mofu-mitsu.github.io/kaomoji-maker/\n#顔文字メーカー #地雷系 #みりんてゃ",
        "image": "images/IMG_2965.jpeg",  # 複数の顔文字を並べた画像
        "alt": "顔文字メーカーで作った様々なジャンルの顔文字コレクション"
    },
    {
        "text": "みりんの今日の色、こんなのだったよ…🎨\n触ってる時間で色の深度が変わるの、ちょっと運命感じない？♡\n1658万通りから出てくる、君だけの1色だよ。\n➡ https://mofu-mitsu.github.io/chroma-log/\n#深層心理カラー診断 #みりんてゃ",
        "image": "images/IMG_2966",  # 自分の診断結果カラー画面
        "alt": "深層心理カラー診断の結果画面"
    },
    {
        "text": "ねぇ、これ見て🥹\n触れてた時間でこんな色が浮かび上がるんだよ…\nなんか、自分の感情が可視化された気分。\n➡ https://mofu-mitsu.github.io/chroma-log/\n#カラー診断 #みりんてゃ",
        "image": "images/IMG_2966.jpeg",  # 別の色の診断結果
        "alt": "深層心理カラー診断"
    },
    {
        "text": "cocotteでメモ書いたら、みりんてゃの心がふわっと整理された♡ シンプルすぎて依存しそう…\nhttps://cocotte-simple-memo.vercel.app/\n#みりんてゃ #メモ魔",
        "image": "images/IMG_9041.jpeg",
        "alt": "みりんてゃがcocotteで可愛いメモ書いてるスクショ♡"
    },
    {
        "text": "管理人さんがみりんの学生証つくってくれたの…♡\n2年2組のH2213って数字、なんかかわいくない？🎀\n地雷系でENFPのあざとさもばっちり写ってて、みりん、ちょっと照れてるんだけど🥺💗\n3/20生まれでB型で東京の子って、覚えてくれたら…うれしいな。\n➡ https://mofu-mitsu.github.io/Character-Student-ID-Factory/\n#みりんてゃ #学生証 #地雷系 #ENFP女子",
        "image": "images/IMG_9571.png",
        "alt": "キャラ学生証ファクトリーで作った地雷系ENFPのみりんてゃの学生証画像"
    },
    {
        "text": "オリキャラのプロフィール、ちゃんと\"形\"にしてあげると、なんかその子のこと、もっと好きになるんだよね🫶\nみりんてゃのプロフィール、こんな感じに仕上がりました♡\n➡ https://mofu-mitsu.github.io/orikyara-profile-maker2/\n#オリキャラ #みりんてゃ",
        "image": "images/IMG_2426.png",  # みりんてゃプロフィール画像
        "alt": "オリキャラプロフィールメーカー2で作ったみりんてゃのプロフィール画像"
    },
    {
        "text": "冷蔵庫に卵とキャベツしかない日、ある…？🥲\n先駆シェフに丸投げしたら、ちゃんと献立提案してくれたよ🍳✨\nもう一人で悩まなくていいの、これ革命じゃない？\n➡ https://mofu-mitsu.github.io/kondate-maker/\n#献立メーカー #献立",
        "image": "images/IMG_2973.jpeg",  # 献立メーカーの結果画面
        "alt": "先駆シェフの献立メーカーで提案された献立画面"
    },

    {
        "text": "キーワードからエモい言葉になるの、ちょっと魔法みたいじゃない？🌙\n気持ち言えない夜に、みりんこれ結構すき…♡\n➡ https://emopost-maker.pages.dev/\n#ポエム #みりんてゃ",
        "image": "images/mirintya_poem_1778403164772.png",  # エモポストの生成結果スクショ
        "alt": "今夜のエモポストメーカーで生成されたエモい文章の結果画面"
    },
    {
        "text": "冷蔵庫の中身だけでごはん考えるの無理〜って日に、先駆シェフ呼べるの助かる🍳\n➡ https://mofu-mitsu.github.io/kondate-maker/\n#献立 #みりんてゃ",
        "image": "images/IMG_2972.jpeg",  # 食材入力→結果が見えるスクショ
        "alt": "先駆シェフの献立メーカーで食材から献立を提案している画面"
    },
    {
        "text": "うちの子の設定、ちゃんと形にすると急に愛着増すんだよね🎀\nみりんてゃのプロフィール、こんな感じ♡\n➡ https://mofu-mitsu.github.io/orikyara-profile-maker2/\n#オリキャラ #みりんてゃ",
        "image": "images/IMG_2426.png",  # プロフィール完成画像
        "alt": "オリキャラプロフィールメーカー2で作った完成プロフィール画像"
    },
    {
        "text": "学生証って、なんかロマンあるよね…？🥹\n学校名もロゴも自由に作れるから、うちの子を学園に通わせたくなるの♡\n➡ https://mofu-mitsu.github.io/Character-Student-ID-Factory/\n#創作 #みりんてゃ",
        "image": "images/IMG_9571.png",  # 学生証完成画像
        "alt": "キャラ学生証ファクトリーで作ったオリジナル学生証の完成画像"
    },
    {
        "text": "みりんてゃ、ついに声を手に入れたの🎤💕\n黒猫っぽくてちょいあざとめを目指したUTAU音源だよ♡\n➡ https://torisproject.booth.pm/items/8268069\n#UTAU #みりんてゃ",
        "image": "images/IMG_2716.png",  # BOOTHページや音源紹介画像のスクショ
        "alt": "地雷系黒猫みりんてゃのUTAU音源紹介画像"
    },
    {
        "text": "顔文字って、自分だけの\"顔\"みたいでかわいいよね🎀\nパーツ組み合わせて遊べるの、ずっとやっちゃう…♡\n➡ https://mofu-mitsu.github.io/kaomoji-maker/\n#顔文字 #みりんてゃ",
        "image": "images/IMG_2963.jpeg",  # 作成した顔文字のスクショ
        "alt": "顔文字メーカーで作ったみりんてゃの可愛い顔文字"
    },
    {
        "text": "みりんてゃの世界、実は診断もゲームもメーカーも結構あるんだよ🫶\n気分でのぞいて好きなの見つけてって〜♡\n➡ https://mofu-mitsu.github.io/contents.html\n#創作ツール #みりんてゃ",
        "image": "images/IMG_2974.png",  # ポータル一覧のスクショ
        "alt": "Torinooka World公式ポータルに並んだ複数のツール一覧画面"
    }
    # ここに10〜20個追加（オリキャラ、夢日記とかも）
    # 画像は事前にツール開いて、みりんてゃ風に入力→スクショ→images/に保存
]

# ------------------------------
# ★ 画像アップロード (圧縮対応)
# ------------------------------
def upload_image(client, image_path):
    img = Image.open(image_path)
    max_dimension = 1024
    if max(img.size) > max_dimension:
        ratio = max_dimension / max(img.size)
        new_size = (int(img.width * ratio), int(img.height * ratio))
        img = img.resize(new_size, Image.LANCZOS)

    buffer = io.BytesIO()
    quality = 95
    while True:
        buffer.seek(0)
        buffer.truncate(0)
        img.convert("RGB").save(buffer, format="JPEG", quality=quality, optimize=True)
        if buffer.tell() / 1024 <= 976 or quality <= 20:
            break
        quality -= 5
    buffer.seek(0)
    return client.com.atproto.repo.upload_blob(buffer.read()).blob

# ------------------------------
# ★ OGP embed
# ------------------------------
def generate_embed_from_url(client, url):
    try:
        headers = {'User-Agent': 'Mozilla/5.0'}
        response = requests.get(url, headers=headers, timeout=10)
        if response.status_code != 200:
            return None
        soup = BeautifulSoup(response.text, 'html.parser')
        title = soup.find("meta", property="og:title") or soup.find("title")
        description = soup.find("meta", property="og:description")
        image = soup.find("meta", property="og:image")

        title = (title["content"] if title and "content" in title.attrs else title.string) if title else "もふみつ工房♡"
        description = description["content"] if description else "ふわもこツールで遊んでみて♡"
        image_url = image["content"] if image else None

        thumb_blob = None
        if image_url:
            try:
                img_res = requests.get(image_url, timeout=10)
                if img_res.status_code == 200 and len(img_res.content) < 1000000:
                    thumb_blob = client.com.atproto.repo.upload_blob(img_res.content).blob
            except:
                pass

        external = {"uri": url, "title": title[:300], "description": description[:300]}
        if thumb_blob:
            external["thumb"] = thumb_blob
        return {"$type": "app.bsky.embed.external", "external": external}
    except:
        return None

# ------------------------------
# ★ facets & 正規化
# ------------------------------
def generate_facets_from_text(text, hashtags):
    text_bytes = text.encode("utf-8")
    facets = []
    for tag in hashtags:
        tag_bytes = tag.encode("utf-8")
        start = text_bytes.find(tag_bytes)
        if start != -1:
            facets.append({
                "index": {"byteStart": start, "byteEnd": start + len(tag_bytes)},
                "features": [{"$type": "app.bsky.richtext.facet#tag", "tag": tag.lstrip("#")}]
            })
    url_pattern = r'(https?://[^\s]+)'
    for match in re.finditer(url_pattern, text):
        url = match.group(0)
        url_bytes = url.encode("utf-8")
        start = text_bytes.find(url_bytes)
        if start != -1:
            facets.append({
                "index": {"byteStart": start, "byteEnd": start + len(url_bytes)},
                "features": [{"$type": "app.bsky.richtext.facet#link", "uri": url}]
            })
    return facets

def normalize_text(text):
    return unicodedata.normalize("NFKC", text).strip()

# ------------------------------
# ★ メイン投稿 (修正版: 画像時はOGP無効化)
# ------------------------------
client = Client()
client.login(HANDLE, APP_PASSWORD)

# ランダム選択：70%テキストだけ、30%画像付き
if random.random() < 0.3 and IMAGE_POSTS:
    post_data = random.choice(IMAGE_POSTS)
    message = normalize_text(post_data["text"])
    image_blob = upload_image(client, post_data["image"])
    embed = {
        "$type": "app.bsky.embed.images",
        "images": [{"image": image_blob, "alt": post_data["alt"]}]
    }
    # ★ 画像時はOGPスキップ！ URLはfacetsでリンク化されるからOK
    # URLのfacetsはgenerate_facetsで自動処理されるよ
else:
    raw_message = random.choice(POST_MESSAGES)
    message = normalize_text(raw_message)
    embed = None
    # URLあればOGP (テキスト時だけ)
    url_match = re.search(r'(https?://[^\s]+)', message)
    if url_match:
        embed = generate_embed_from_url(client, url_match.group(0))

hashtags = [word for word in message.split() if word.startswith("#")]
facets = generate_facets_from_text(message, hashtags)

client.send_post(text=message, facets=facets if facets else None, embed=embed)
